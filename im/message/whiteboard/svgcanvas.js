...
(function() {
	...
	jQuery.fn.attr = function(key, value) {
		...
		for(var i=0; i<len; i++) {
			...
			if(elem.namespaceURI === svgns||$(elem).closest('foreignObject').length>0) {
				// Setting attribute
				if(value !== undefined) {
...
var curConfig = {
	...
	selectNew: false,//select the element once created imcustomize!!!yangjz0
	dimensions: [640, 480]
};
...
$('#whbd_container')[0].appendChild(svgroot);
...
var addSvgElementFromJson = this.addSvgElementFromJson = function(data) {
	...
	if (!shape) {
		...
		if (current_layer) {
			if(shape.tagName=='foreignobject'){
					var a = function(a) {
		            ...
		            shape1 = (!svgCanvas.baloonType)?a('<foreignObject overflow="visible" width="200" opacity="1" height="200" requiredFeatures="http://www.w3.org/TR/SVG11/feature#Extensibility" x="100" y="100"><div class="clearfix" xmlns="http://www.w3.org/1999/xhtml"><textArea class="text-entity" readonly="true" resizeable=false xmlns="http://www.w3.org/1999/xhtml" style="width:200px;height:200px;font-size:'+cur_text.font_size+'px;font-family:'
							+cur_text.font_family+';color:#'+svgCanvas.textFill+';" ></textArea></div></foreignObject>'):
		            		a('<foreignObject data-eleType="'+(svgCanvas.baloonType||'text')+'"  overflow="visible" style="overflow:visible;" width="200" opacity="1" height="200" class="clearfix" requiredFeatures="http://www.w3.org/TR/SVG11/feature#Extensibility" x="100" y="100"><div class="clearfix" xmlns="http://www.w3.org/1999/xhtml"><textArea class="text-entity" readonly="true" resizeable=false xmlns="http://www.w3.org/1999/xhtml" style="width:200px;height:100px;font-size:'+cur_text.font_size+'px;font-family:'
							+cur_text.font_family+';color:#'+svgCanvas.textFill+';" ></textArea><span class="nth1" xmlns="http://www.w3.org/1999/xhtml"></span><span class="nth2" xmlns="http://www.w3.org/1999/xhtml"></span></div></foreignObject>');
//		            shape = shape1.firstElementChild;
		            shape = $(shape1).children()[0]
//				$(current_layer)[0].appendChild(shape);
				$(current_layer).append(shape);
			}else{
...
//	console.log(">>>>>",cmd);//imcustomize
	if(window.changeState)changeState(cmd,0);
	if(cmd.text=="Create foreignObject"&&$(cmd.elem).find('textarea').html().length==0){
		return false;
	}
...
var getMouseTarget = this.getMouseTarget = function(evt) {
	...
	if(mouse_target.tagName=='foreignObject'||$.inArray($(mouse_target).attr('id'),['arr_selector_e','arr_selector_s'])>=0){
...
(function() {
	...
	//   and do nothing else
	var arrSelInitial,txtProxyInitial;
	var mouseDown = function(evt)
	{	
		if(svgCanvas.arrMode&&!$(evt.target).is('.arrow,.arr-cir')){
			$('.im-tool-arrow').trigger('click');
		}
...
// 					console.log('s',[start_x, start_y]);
				...
				if($(mouse_target).is('.arrow')){
					arrSelInitial = {point_s:{cx:$('#arr_selector_s').attr('cx'),cy:$('#arr_selector_s').attr('cy')},point_e:{cx:$('#arr_selector_e').attr('cx'),cy:$('#arr_selector_e').attr('cy')}}
				}
				if($(mouse_target).is('.text-holder')){
					var eleId = $(mouse_target).attr('id'),
						proxy = $('[data-tar="'+eleId+'"]');
					txtProxyInitial = {
						'left':parseFloat(proxy.css('left').replace('px','')),
						'top':parseFloat(proxy.css('top').replace('px',''))
					}
				}
				if($(mouse_target).is('.arr-cir')){
					$(mouse_target).addClass('sel')
				}
				break;
			case "zoom": 
				started = true;
			...
			case "triangle":
				...
				start_x = x;
				start_y = y;
				current_d= "M395.5264293124419,379.0067138671875 L414.9554677694614,359.5781421661377 L434.38372802734375,379.0067138671875 L395.5264293124419,379.0067138671875 z";
				cur_shape = addSvgElementFromJson({
					"element": "path",
					"curStyles": true,
					"attr": {
						"x": x,
					...
					if(/[a-z]/.test(current_d)) {
						current_d = canvas.pathActions.convertPath(cur_shape);
						cur_shape.setAttribute('d', current_d);
						canvas.pathActions.fixEnd(cur_shape);
					}
			
					cur_shape.setAttribute('transform', "translate(" + x + "," + y + ") scale(0.005) translate(" + -x + "," + -y + ")");
					
					canvas.recalculateDimensions(cur_shape);
					
					var tlist = canvas.getTransformList(cur_shape);
			...
			case "baloon":
				...
				start_x = x;
				start_y = y;
				var dArr = ["M589.89783,324.43353L589.89783,324.43353C589.89783,322.35962 591.53931,320.67929 593.56445,320.67929L595.23157,320.67929L595.23157,320.67929L603.23132,320.67929L618.2312,320.67929C619.20453,320.67929 620.13574,321.07449 620.82446,321.77899C621.51154,322.48312 621.89783,323.43802 621.89783,324.43353L621.89783,333.82056L621.89783,333.82056L621.89783,339.45248L621.89783,339.45248C621.89783,341.52591 620.25714,343.20709 618.2312,343.20709L603.23132,343.20709L592.78003,352.75482L595.23157,343.20709L593.56445,343.20709C591.53931,343.20709 589.89783,341.52591 589.89783,339.45248L589.89783,339.45248L589.89783,333.82056L589.89783,333.82056L589.89783,324.43353z",
				           "M176.538599,143.98553466796875 L176.5385995609313,143.98553466796875 C176.5385995609313,140.92381286621094 174.0874162529326,138.44735717773438 171.06436814796325,138.44735717773438 L168.574459881861,138.44735717773438 L168.574459881861,138.44735717773438 L156.63051240725963,138.44735717773438 L134.2369638726268,138.44735717773438 C132.7835890195911,138.44735717773438 131.3939605738642,139.0282745361328 130.366498497674,140.0681915283203 C129.33913678521094,141.10841369628906 128.7628173828125,142.51754760742188 128.7628173828125,143.98553466796875 L128.7628173828125,157.83509826660156 L128.7628173828125,157.83509826660156 L128.7628173828125,166.14231872558594 L128.7628173828125,166.14231872558594 C128.7628173828125,169.20399475097656 131.21170776566032,171.68482971191406 134.2369638726268,171.68482971191406 L156.63051240725963,171.68482971191406 L172.23534263368038,185.76870727539062 L168.574459881861,171.68482971191406 L171.06436814796325,171.68482971191406 C174.0874162529326,171.68482971191406 176.5385995609313,169.20399475097656 176.5385995609313,166.14231872558594 L176.5385995609313,166.14231872558594 L176.5385995609313,157.83509826660156 L176.5385995609313,157.83509826660156 L176.5385995609313,143.98553466796875 z",
				           "M76.97172855753888,231.89976501464844 L76.97172855753888,231.89976501464844 C76.97172855753888,235.28009033203125 79.62892496613131,238.0142364501953 82.90605281813623,238.0142364501953 L85.60521427480853,238.0142364501953 L85.60521427480853,238.0142364501953 L98.5529947517316,238.0142364501953 L122.82859236947115,238.0142364501953 C124.40412601730714,238.0142364501953 125.91053395133713,237.37286376953125 127.0243528230385,236.22474670410156 C128.13805590627453,235.07627868652344 128.7628173828125,233.5205078125 128.7628173828125,231.89976501464844 L128.7628173828125,216.60903930664062 L128.7628173828125,216.60903930664062 L128.7628173828125,207.4373779296875 L128.7628173828125,207.4373779296875 C128.7628173828125,204.05711364746094 126.10811869682948,201.31813049316406 122.82859236947115,201.31813049316406 L98.5529947517316,201.31813049316406 L81.63666387265039,185.76870727539062 L85.60521427480853,201.31813049316406 L82.90605281813623,201.31813049316406 C79.62892496613131,201.31813049316406 76.97172855753888,204.05711364746094 76.97172855753888,207.4373779296875 L76.97172855753888,207.4373779296875 L76.97172855753888,216.60903930664062 L76.97172855753888,216.60903930664062 L76.97172855753888,231.89976501464844 z",
				           "M176.53860473632812,231.89976435920107 L176.53860473632812,231.89976435920107 C176.53860473632812,235.28008362166292 174.08741760253906,238.01423499413067 171.06436157226562,238.01423499413067 L168.574462890625,238.01423499413067 L168.574462890625,238.01423499413067 L156.63050842285156,238.01423499413067 L134.23696899414062,238.01423499413067 C132.78358459472656,238.01423499413067 131.3939666748047,237.37286838291038 130.3665008544922,236.2247394320584 C129.33914184570312,235.07627354936267 128.7628173828125,233.520507607318 128.7628173828125,231.89976435920107 L128.7628173828125,216.60903919198608 L128.7628173828125,216.60903919198608 L128.7628173828125,207.43738377936825 L128.7628173828125,207.43738377936825 C128.7628173828125,204.05711505668296 131.21170043945312,201.31812871225702 134.23696899414062,201.31812871225702 L156.63050842285156,201.31812871225702 L172.23533630371094,185.76870727539062 L168.574462890625,201.31812871225702 L171.06436157226562,201.31812871225702 C174.08741760253906,201.31812871225702 176.53860473632812,204.05711505668296 176.53860473632812,207.43738377936825 L176.53860473632812,207.43738377936825 L176.53860473632812,216.60903919198608 L176.53860473632812,216.60903919198608 L176.53860473632812,231.89976435920107 z",
				            ];
				current_d= dArr[window.baloonType?window.baloonType:0];
				cur_shape = addSvgElementFromJson({
					"element": "path",
					"curStyles": true,
					"attr": {
...
//						"opacity": cur_shape.opacity / 2
			...
			case "arrow":
				...
				angleTem = 0;
				disHis = 0;
				current_d = "M 245.32,251.73 L 245.32,251.73 L 243.13,248.10 L 420.41,278.98 L 414.89,259.60 L 465.96,303.10 L 400.52,317.85 L 414.42,303.25 z";
//				current_d = "M "+start_x+","+start_y+" L "+start_x+","+start_y+" C 243.86,251.37 242.77,249.56 243.13,248.10 L 243.13,248.10 C 243.49,246.64 245.30,245.55 246.76,245.91 L 420.41,278.98 L 414.89,259.60 L 465.96,303.10 L 400.52,317.85 L 414.42,303.25 z";
				cur_shape = addSvgElementFromJson({
					"element": "path",
					"curStyles": true,
					"attr": {
			...
			case "line":
				...
				addSvgElementFromJson({
					...
					"attr": {
						...
						"y2": y,
						"id": getNextId(),
//						"stroke": "#000000",//cur_shape.stroke,
...
//						"fill": "none",
			...
			case "focus":
				...
				if($('.focus-cir').length==0){
					...
					$('.focus-cir').show().attr({'cx':x,'cy':y});
				}
				svgCanvas.addToSelection($('.focus-cir'),false);
				svgCanvas.moveToTopSelectedElement();
				break;
			case "textarea":
				started = true;
				var obj = {
					"element": svgCanvas.isIE?"rect":"foreignobject",
					"curStyles": false,
					"attr": {
						"overflow":'visible',
//						"requiredFeatures": "http://www.w3.org/TR/SVG11/feature#Extensibility",
						"height":svgCanvas.baloonType?100:200,
						"width":200,
						"opacity":1,
						"x": x,
						"y": y,
						"fill":"none",
						"id": getNextId(),
						"class":!svgCanvas.isIE?"":!!svgCanvas.baloonType?"text-holder baloon baloon-"+svgCanvas.baloonType:"text-holder"
					}
				}
				if(svgCanvas.isIE&&svgCanvas.baloonType)obj["attr"]["data-eletype"]=svgCanvas.baloonType;
				var newText = addSvgElementFromJson(obj);
				break;
			case "text":
...
// 					newText.textContent = "text";
	...
	canvas.latestMouseEvt = null;
	var mouseMove = function(evt)
	{	
		if (!started) return;
		canvas.latestMouseEvt = evt; 
		...
		if($('.arr-cir.sel').length>0)$('.arr-cir.sel').attr({cx:x,cy:y});
		evt.preventDefault();
		var cur_tar = $(evt.target);
		switch (current_mode)
		{
			case "select":
...
//							}
							if($(selected).is('.arrow')&&cur_tar.is('.arr-cir')){
								cur_tar[0].setAttribute('class','arr-cir sel');//addClass('sel');

								current_d = "M 245.32,251.73 L 245.32,251.73 L 243.13,248.10 L 420.41,278.98 L 414.89,259.60 L 465.96,303.10 L 400.52,317.85 L 414.42,303.25 z";
								
								var x0 = cur_tar.is('#arr_selector_e')?arrSelInitial['point_s'].cx:arrSelInitial['point_s'].cx+dx,
									y0 = cur_tar.is('#arr_selector_e')?arrSelInitial['point_s'].cy:arrSelInitial['point_s'].cy+dy,
									x1 = cur_tar.is('#arr_selector_e')?arrSelInitial['point_e'].cx+dx:arrSelInitial['point_e'].cx,
									y1 = cur_tar.is('#arr_selector_e')?arrSelInitial['point_e'].cy+dy:arrSelInitial['point_e'].cy,
									curDis = Math.getCurDis(x0,y0,x1,y1),
									ratio = curDis/380,
									points = curDis>380?[{'x':x0-1.4,'y':y0-0.5},{'x':x0-2.34,'y':y0-2.4},{'x':x0-1.85,'y':y0-3.82},{'x':x0-1.35,'y':y0-5.24},{'x':x0+0.56,'y':y0-6.16},{'x':x0+1.97,'y':y0-5.67},{'x':x0+curDis-60,'y':y0-15},{'x':x0+curDis-80,'y':y0-40},{'x':x0+curDis-80,'y':y0+40},{'x':x0+curDis-60,'y':y0+15}]:
										[{'x':x0-(1.4*ratio),'y':y0-(0.5*ratio)},{'x':x0-(2.34*ratio),'y':y0-(2.4*ratio)},{'x':x0-(1.85*ratio),'y':y0-(3.82*ratio)},{'x':x0-(1.35*ratio),'y':y0-(5.24*ratio)},{'x':x0+(0.56*ratio),'y':y0-(6.16*ratio)},{'x':x0+(1.97*ratio),'y':y0-(5.67*ratio)},{'x':x0+curDis-(60*ratio),'y':y0-(15*ratio)},{'x':x0+curDis-(80*ratio),'y':y0-(40*ratio)},{'x':x0+curDis-(80*ratio),'y':y0+(40*ratio)},{'x':x0+curDis-(60*ratio),'y':y0+(15*ratio)}],
										rotateAngle = Math.getRotateDeg(x0,y0,x1,y1),
										rPoints = [Math.getRotatedPoint({'x':x0,'y':y0},points[0],rotateAngle),
										           Math.getRotatedPoint({'x':x0,'y':y0},points[1],rotateAngle),
										           Math.getRotatedPoint({'x':x0,'y':y0},points[2],rotateAngle),
										           Math.getRotatedPoint({'x':x0,'y':y0},points[3],rotateAngle),          
										           Math.getRotatedPoint({'x':x0,'y':y0},points[4],rotateAngle),
										           Math.getRotatedPoint({'x':x0,'y':y0},points[5],rotateAngle),
										           Math.getRotatedPoint({'x':x0,'y':y0},points[6],rotateAngle),         
										           Math.getRotatedPoint({'x':x0,'y':y0},points[7],rotateAngle),
										           Math.getRotatedPoint({'x':x0,'y':y0},points[8],rotateAngle),
										           Math.getRotatedPoint({'x':x0,'y':y0},points[9],rotateAngle)];          
									current_d = "M "+x0+","+y0+" L "+x0+","+y0+//閻愶拷
									" C "+rPoints[0]['x']+","+rPoints[0]['y']+" "+rPoints[1]['x']+","+rPoints[1]['y']+" "+rPoints[2]['x']+","+rPoints[2]['y']+
									" L "+rPoints[2]['x']+","+rPoints[2]['y']+
									" C "+rPoints[3]['x']+","+rPoints[3]['y']+" "+rPoints[4]['x']+","+rPoints[4]['y']+" "+rPoints[5]['x']+","+rPoints[5]['y']+
									" L "+rPoints[6]['x']+","+rPoints[6]['y']+//閻愰�绨�
									" L "+rPoints[7]['x']+","+rPoints[7]['y']+//閻愰�绗�
									" L "+x1+","+y1+//閻愮懓娲�缁狀厼銇旀い鍓佸仯)
									" L "+rPoints[8]['x']+","+rPoints[8]['y']+//閻愰�绨�
									" L "+rPoints[9]['x']+","+rPoints[9]['y']+//閻愮懓鍙�
									" L "+x0+","+y0+" z";//闂傤厼鎮庣捄顖氱窞閸ョ偛鍩岀挧椋庡仯
									selected.setAttribute('d', current_d);
							}else if($('.arr-cir.sel').length==0){
								if($(selected).is('.arrow')){
									$('#arr_selector_s').attr({cx:arrSelInitial['point_s'].cx+dx,cy:arrSelInitial['point_s'].cy+dy});
									$('#arr_selector_e').attr({cx:arrSelInitial['point_e'].cx+dx,cy:arrSelInitial['point_e'].cy+dy});
								}
								if($(selected).is('.text-holder')){
									var node_r = $('#svgcanvas')[0],
										ldis_r = node_r.getBoundingClientRect().left,
										tdis_r = node_r.getBoundingClientRect().top,
										para = $('#selectedBox0')[0].getBoundingClientRect(),
										proxy =$('[data-tar="'+$(selected).attr('id')+'"]'); 
									proxy.css({'left':para.left-ldis_r,'top':para.top-tdis_r});
								}
				...
				break;
			case "multiselect":
//				return false; //绂佺敤鎷栭� imcustomize!!!yangjz0
				real_x *= current_zoom;
				real_y *= current_zoom;
				assignAttributes(rubberBox, {
			...
			case "resize":
				...
				//文字resize
				if($(selected).find('textarea').length||$(selected).is('.text-holder')){
					if(width*sx*svgCanvas.zoomRatio<100||height*sy*svgCanvas.zoomRatio<55){return false;}
					var node = $('#selectedBox0')[0],
					ldis = node.getBoundingClientRect().left,
					tdis = node.getBoundingClientRect().top;
					$(selected).find('.nth1').css('width',width*sx-80)
					if($(selected).is('[data-eletype]')&&$(selected).attr('data-eletype')>1){
						$(selected).find('.nth1').css({'margin-top':-(height*sy)-3+'px'});
						$(selected).find('.nth2').css({'margin-top':-(height*sy)-3+'px'});
//						$(selected).find('.nth1').css({'margin-top':-(height*sy+100-1)+'px','margin-bottom':(height*sy+100-1)+'px'});
//						$(selected).find('.nth2').css({'margin-top':-(height*sy+100-10)+'px','margin-bottom':(height*sy+100-10)+'px'});
					}
						$(selected).find('*').hide();
//					$('#text_area').attr('dataLink',$(selected).attr('id')).css({left:ldis,top:tdis,width:width*sx*svgCanvas.zoomRatio,height:height*sy*svgCanvas.zoomRatio,pointerEvents:'none'});				
					$('#textarea_proxy').removeClass().addClass($(selected).is('[data-eletype]')?('baloon-input baloon-input'+($(selected).attr('data-eletype'))):'').attr('dataLink',$(selected).attr('id')).css({left:ldis,top:tdis,width:width*sx*svgCanvas.zoomRatio>100?width*sx*svgCanvas.zoomRatio:100,height:height*sy*svgCanvas.zoomRatio,pointerEvents:'none'});				
				}
...
//				if(y < start_y) ty = lastBBox.height;
				...
				translateOrigin.setTranslate(-(left+tx), -(top+ty));
//				translateOrigin.setTranslate(start_x, start_y);
				var realDis = Math.getCurDis(start_x, start_y, real_x, real_y);
				var sacleType = realDis-disHis;
				if(!evt.shiftKey) {
					var max = sacleType>=0?Math.max(Math.abs(sx), Math.abs(sy)):Math.min(Math.abs(sx), Math.abs(sy));
	
//					sx = max * (sx < 0 ? -1 : 1);
//					sy = max * (sy < 0 ? -1 : 1);
					sx = disHis==0?2:realDis/disHis;
					sy = disHis==0?2:realDis/disHis;
				}
				
//				console.log("sssssssssssssssssssssss",sx,sy,realDis/disHis,realDis/disHis);
//				scale.setScale(realDis/disHis,realDis/disHis);
				scale.setScale(sx,sy);
				
				translateBack.setTranslate(left+tx, top+ty);
				disHis = realDis;
//				debugger;
//				var newTrans = svgroot.createSVGTransform();
//				newTrans.setRotate(1,0,0);
				
//				console.log("sssssssssssssssssssssss",[start_x,start_y]);
				var N = tlist.numberOfItems;
				var curAngle = Math.getRotateDeg(start_x,start_y,real_x,real_y)
				var angleDif = curAngle-angleTem;
				angleTem = curAngle;
				newrot.setRotate(angleDif, start_x, start_y);
//				if(cur_shape.getAttributeNS(null,'transform')&&cur_shape.getAttributeNS(null,'transform').indexOf('rotate')>=0){
...
//				tlist.appendItem(translateOrigin);
				...
				var x0 = start_x,
			    	y0 = start_y,
			    	curDis = Math.getCurDis(x0,y0,real_x,real_y),
			    	ratio = curDis/380,
				
				points = curDis>380?[{'x':x0-1.4,'y':y0-0.5},{'x':x0-2.34,'y':y0-2.4},{'x':x0-1.85,'y':y0-3.82},{'x':x0-1.35,'y':y0-5.24},{'x':x0+0.56,'y':y0-6.16},{'x':x0+1.97,'y':y0-5.67},{'x':x0+curDis-60,'y':y0-15},{'x':x0+curDis-80,'y':y0-40},{'x':x0+curDis-80,'y':y0+40},{'x':x0+curDis-60,'y':y0+15}]:
					[{'x':x0-(1.4*ratio),'y':y0-(0.5*ratio)},{'x':x0-(2.34*ratio),'y':y0-(2.4*ratio)},{'x':x0-(1.85*ratio),'y':y0-(3.82*ratio)},{'x':x0-(1.35*ratio),'y':y0-(5.24*ratio)},{'x':x0+(0.56*ratio),'y':y0-(6.16*ratio)},{'x':x0+(1.97*ratio),'y':y0-(5.67*ratio)},{'x':x0+curDis-(60*ratio),'y':y0-(15*ratio)},{'x':x0+curDis-(80*ratio),'y':y0-(40*ratio)},{'x':x0+curDis-(80*ratio),'y':y0+(40*ratio)},{'x':x0+curDis-(60*ratio),'y':y0+(15*ratio)}],
			              rotateAngle = Math.getRotateDeg(start_x,start_y,real_x,real_y),
			    rPoints = [Math.getRotatedPoint({'x':x0,'y':y0},points[0],rotateAngle),
			               Math.getRotatedPoint({'x':x0,'y':y0},points[1],rotateAngle),
			               Math.getRotatedPoint({'x':x0,'y':y0},points[2],rotateAngle),
			               Math.getRotatedPoint({'x':x0,'y':y0},points[3],rotateAngle),          
							...
							Math.getRotatedPoint({'x':x0,'y':y0},points[8],rotateAngle),
							Math.getRotatedPoint({'x':x0,'y':y0},points[9],rotateAngle)];          
			current_d = "M "+start_x+","+start_y+" L "+start_x+","+start_y+//閻愶拷
						" C "+rPoints[0]['x']+","+rPoints[0]['y']+" "+rPoints[1]['x']+","+rPoints[1]['y']+" "+rPoints[2]['x']+","+rPoints[2]['y']+
						" L "+rPoints[2]['x']+","+rPoints[2]['y']+
						" C "+rPoints[3]['x']+","+rPoints[3]['y']+" "+rPoints[4]['x']+","+rPoints[4]['y']+" "+rPoints[5]['x']+","+rPoints[5]['y']+
						" L "+rPoints[6]['x']+","+rPoints[6]['y']+//閻愰�绨�
						" L "+rPoints[7]['x']+","+rPoints[7]['y']+//閻愰�绗�
						" L "+real_x+","+real_y+//閻愮懓娲�缁狀厼銇旀い鍓佸仯)
						" L "+rPoints[8]['x']+","+rPoints[8]['y']+//閻愰�绨�
						" L "+rPoints[9]['x']+","+rPoints[9]['y']+//閻愮懓鍙�
						" L "+start_x+","+start_y+" z";//闂傤厼鎮庣捄顖氱窞閸ョ偛鍩岀挧椋庡仯
...
// 					}
	...
	{   
		svgCanvas.latestMouseEvt = null;
		$('.arr-cir.sel').length&&$('.arr-cir.sel')[0].setAttribute('class','arr-cir');
				...
				if (selectedElements[0] != null) {
					...
					if (selectedElements[1] == null) {
						...
						switch ( selected.tagName ) {
							...
							case "image":
							case "foreignObject":
								if($(selected).find('textarea').length){
									var box = svgCanvas.getBBox(selected),
										node = $('#selectedBox0')[0]
										ldis = node.getBoundingClientRect().left,
										tdis = node.getBoundingClientRect().top;
//										$('#textarea_proxy').removeClass().addClass(svgCanvas.baloonType||($(selected).find('[data-eletype]').length>0)?('baloon-input baloon-input'+(svgCanvas.baloonType||$(selected).find('[data-eletype]').attr('data-eletype'))):'').attr('dataLink',$(selected).attr('id')).css({pointerEvents:'inherit'}).show().find('textarea').val($(selected).find('textarea').html());
										$('#textarea_proxy').removeClass().addClass($(selected).is('[data-eletype]')?('baloon-input baloon-input'+($(selected).attr('data-eletype'))):'').attr('dataLink',$(selected).attr('id')).css({pointerEvents:'inherit'}).show().find('textarea').val($(selected).find('textarea').html());
										if(current_mode=='select'&&!isResize){
											var cssObj = {left:ldis,top:tdis,width:$(selected).attr('width')*svgCanvas.zoomRatio,
													height:$(selected).attr('height')*svgCanvas.zoomRatio,
													fontFamily:$(selected).find('textarea').css('fontFamily')||cur_text.font_family,
													fontSize:Math.round(parseInt(($(selected).find('textarea').css('fontSize')||cur_text.font_size).replace('px',''),10)*svgCanvas.zoomRatio)+'px',
													color:$(selected).find('textarea').css('color')||svgCanvas.textFill||'#f00'};
											canvas.setColorGuide($(selected).find('textarea').css('color').replace('#',''));
											$('#textarea_proxy').css(cssObj);
//											$('#textarea_proxy').removeClass().addClass(svgCanvas.baloonType?('baloon-input baloon-input'+svgCanvas.baloonType):'').css({left:ldis,top:tdis,width:$(selected).width()*svgCanvas.zoomRatio,height:$(selected).height()*svgCanvas.zoomRatio,fontFamily:cur_text.font_family,fontSize:cur_text.font_size*svgCanvas.zoomRatio,color:$(selected).find('textarea').css('color')||svgCanvas.textFill||'#f00'});
										}
										if(isResize){
											var squObj =  {width:$('#textarea_proxy').width()/svgCanvas.zoomRatio,height:$('#textarea_proxy').height()/svgCanvas.zoomRatio};
											$(selected).find('textarea').css(squObj);

										}
										$(selected).find('*').hide()//select a textarea to edit
										setTimeout(function(){$(selected).find('*').show()},0)
										$(selected).find('textarea').css('color','transparent');//select a textarea to edit
										$('.fz-guide').html($('#textarea_proxy').css('font-size'));
										$('.ff-guide').html(svgCanvas.fontfamilyMap[$('#textarea_proxy').css('font-family').replace(/\'/g,'')]);
										canvas.setMode('textareaedit');
								}
							...
							default:
								if($(selected).is('.text-holder')){
									...
									var box = svgCanvas.getBBox(selected),
										...
										tdis = node.getBoundingClientRect().top
										proxy = $('[data-tar="'+$(selected).attr('id')+'"]');
										$('#textarea_proxy').removeClass().addClass($(selected).is('[data-eletype]')?('baloon-input baloon-input'+($(selected).attr('data-eletype'))):'').attr('dataLink',$(selected).attr('id')).css({pointerEvents:'inherit'}).show().find('textarea').val(proxy.find('textarea').val());
										if(current_mode=='select'&&!isResize){
											
											$('#textarea_proxy').css({left:ldis,top:tdis,width:$(selected).attr('width')*svgCanvas.zoomRatio,height:$(selected).attr('height')*svgCanvas.zoomRatio,fontFamily:cur_text.font_family,fontSize:cur_text.font_size*svgCanvas.zoomRatio,color:$(selected).find('textarea').css('color')||svgCanvas.textFill||'#f00'});
										}
										if(isResize){
											var squObj =  {width:$('#textarea_proxy').width(),height:$('#textarea_proxy').height()};
											proxy.css(squObj)

										}
										proxy.hide();//select a textarea to edit
										$('.fz-guide').html($('#textarea_proxy').css('font-size'));
										$('.ff-guide').html(svgCanvas.fontfamilyMap[$('#textarea_proxy').css('font-family').replace(/\'/g,'')]);
										canvas.setMode('textareaedit');
...
//						else if (evt.shiftKey) {//imcustomize!!
						else if (evt.ctrlKey) {
					...
					if(svgedit.browser.supportsNonScalingStroke()) {
						...
						if (elem) {
							...
							if(!elem.tagName.toLocaleLowerCase()=='foreignobject')
				...
				if($(evt.target).is('.arr-cir')){
					addCommandToHistory(new ChangeElementCommand(selectedElements[0]));
				}
				return;
				break;
			case "zoom":
			...
			case "fhpath":
				...
				}//return{keep:false,element:element,started:false};
				break;
			case "baloon":	
			case "triangle":	
			case "arrow":	//arrow_mouseup
			
				
					var mode = canvas.getMode();
					//if(mode !== mode_id) return;
					
					if(real_x == start_x && real_y.mouse_y == start_y) {
//						return {
//							keep: false,
//							element: cur_shape,
//							started: false
...
//						}
					...
					if(current_mode=='arrow'){
						if(canvas.getSelectedElems()[0])selectorManager.requestSelector(cur_shape).showGrips(true);
						$('.im-tool-select').trigger('click');
						svgCanvas.arrMode = true;
					}
					started = false;
					break;
			...
			case "ellipse":
				...
				break;
			case "focus":
				keep = true;// (element.getAttribute('r') != 0);
				changeState({elem:$('.focus-cir')[0],text:"change text"},0)
				break;
			case "fhellipse":
			...
			case "text":
				keep = true;
				selectOnly([element]);
				textActions.start(element);//debugger;
				//if(element.tagName=='foreignobject')$('#svgcanvas').html($('#svgcanvas').clone())
				break;
			case "textarea":
				keep = true;
				selectOnly([element]);
				textareaActions.start(element);//debugger;
				//textActions.start(element);//debugger;
				//if(element.tagName=='foreignobject')$('#svgcanvas').html($('#svgcanvas').clone())
				break;
			case "path":
			...
			case "textareaedit":
				...
				element = null;
				$('#text_area').blur();
				textareaActions.mouseUp(evt, mouse_x, mouse_y);
				break;
			case "rotate":
				keep = true;
				element = null;
		...
		if (!keep && element != null) {
			...
			if(opac_ani.beginElement && element.getAttribute('opacity') != cur_shape.opacity) {
				cur_shape.opacity=cur_shape.opacity||1;
			...
			setTimeout(function() {
				...
				if(current_mode === "path") {
					...
					if(curConfig.selectNew||$(element).is('.arrow')) {
...
var textareaActions = canvas.textareaActions = function(){
		function start(ele){
			svgCanvas.newTxtarea = true;
			var box = svgCanvas.getBBox(ele),
				node = $('#selectedBox0')[0]
				ldis = node.getBoundingClientRect().left,
				tdis = node.getBoundingClientRect().top;
//				$(ele).find('*').hide();
				$(ele).find('textarea').css('color','transparent');
				
			$('#textarea_proxy').removeClass().addClass($(ele).is('[data-eletype]')?('baloon-input baloon-input'+($(ele).attr('data-eletype'))):'').css({left:ldis,top:tdis,width:$(ele).attr('width')*svgCanvas.zoomRatio,height:$(ele).attr('height')*svgCanvas.zoomRatio,color:'#'+(svgCanvas.textFill?svgCanvas.textFill:'f00'),fontSize:cur_text.font_size*svgCanvas.zoomRatio,fontFamily:cur_text.font_family}).attr('dataLink',$(ele).attr('id')).show().find('textarea').val('').focus();
			//	workaround to that foreignobject is not supported in IE
			if($(ele).is('.text-holder')){
				var node_r = $('#svgcanvas')[0],
				ldis_r = node_r.getBoundingClientRect().left,
				tdis_r = node_r.getBoundingClientRect().top,
				newEle = $('#textarea_proxy').clone().removeAttr('id datalink');
				newEle.addClass($(ele).is('[data-eletype]')?('baloon baloon'+($(ele).attr('data-eletype'))):'');
				newEle.attr('data-tar',$(ele).attr('id'));
				newEle.css({'left':ldis-ldis_r,'top':tdis-tdis_r,'display':'none'})
				$('#fix_cover').append(newEle);
			}
			svgCanvas.setMode('textareaedit');
			
		}
		function setFontColor(color){
//			$(selectedElements[0]).find('*').css({color:'#'+color});
			$('#textarea_proxy').css('color','#'+color)
		}
		function setFontFamily(ff){
			$(selectedElements[0]).find('textarea').css({fontFamily:ff});
			$('#textarea_proxy').css({fontFamily:ff})
		}
		function setFontSize(fs){
			$(selectedElements[0]).find('textarea').css({fontSize:fs+'px'});
			$('#textarea_proxy').css({fontSize:fs+'px'})
		}
		return{
			start:start,
			...
			mouseUp: function(evt, mouse_x, mouse_y) {
				var box = svgCanvas.getBBox(selectedElements[0]),
					_w = $('#textarea_proxy').width()/svgCanvas.zoomRatio,
					_h = $('#textarea_proxy').height()/svgCanvas.zoomRatio,
					selectedEle = 	$(selectedElements[0]),
					proxy = $('[data-tar="'+selectedEle.attr('id')+'"]');
					if(svgCanvas.isIE){
						proxy.show().find('textarea').css({color:($('#textarea_proxy').css('color')?$('#textarea_proxy').css('color'):'#f00')});
					}else{
						selectedEle.find('*').css({color:($('#textarea_proxy').css('color')?$('#textarea_proxy').css('color'):'#f00')}).show();
					}
					
				if((proxy.length&&proxy.find('textarea').val().length==0)||($(selectedElements[0]).find('textarea').length&&$(selectedElements[0]).find('textarea').html().length<=0)){
					addCommandToHistory(new RemoveElementCommand(selectedElements[0],selectedElements[0].parentNode));
					$(selectedElements[0]).remove();
//					changeState({elem:selectedElements[0],text:'Delete path'},0);
				}else if(svgCanvas.newTxtarea){
					addCommandToHistory(new InsertElementCommand(selectedElements[0]));
				}else{
					addCommandToHistory(new MoveElementCommand(selectedElements[0],selectedElements[0].parentNode));
				}
				svgCanvas.clearSelection();
				svgCanvas.setMode('select');
				$('#textarea_proxy').hide();
				svgCanvas.newTxtarea = false;
...
// 					keep = true;
		...
		mouseMove: function(mouse_x, mouse_y) {
			hasMoved = true;
			if(current_mode === "path"|| current_mode === "arrow") {
...
this.setSvgString = function(xmlString) {
	try {
		...
		if(svgCanvas.isIE)this.resolveIE(newDoc);
		if($(newDoc).find('.focus-cir').length>0)$(newDoc).find('.focus-cir').attr({style:'pointer-events:none;display:none;',fill:'#f00'});
		...
		var attrs = {
			id: 'svgcontent',
//			overflow: curConfig.show_outside_canvas?'visible':'hidden'
				overflow: curConfig.show_outside_canvas?'visible':'visible'
		};
...
// Parameters:
// val - String with the new text
this.setTextAreaContent = function(val) {
	var targetNode = $(selectedElements[0]);
	console.log(selectedElements)
	//selectedElements
	svgCanvas.setMode('textareaedit');
	$(selectedElements[0]).find('textarea').html(val);
	$('[data-tar="'+targetNode.attr('id')+'"]').find('textarea').val(val)
	$('#'+$(selectedElements[0]).attr('id')).replaceWith($(selectedElements))
...
this.setImageURL = function(val) {
	...
	var lastHis = svgCanvas.undoMgr.undoStack[svgCanvas.undoMgr.undoStack.length-1];
	if(lastHis&&(lastHis.text=='position')&&lastHis.stack[0]&&lastHis.stack[0].stack[0]&&($(lastHis.stack[0].stack[0].elem).attr('id')==$(elem).attr('id')))return;
	...
	if(setsize) {
		$(new Image()).load(function() {
			...
			$(elem).attr({
				width: this.height>$('#svgcanvas').height()?this.width*($('#svgcanvas').height()/this.height):this.width,
				height: this.height>$('#svgcanvas').height()?$('#svgcanvas').height():this.height
			});
...
var changeSelectedAttribute = this.changeSelectedAttribute = function(attr, val, elems) {
	var elems = elems || selectedElements;
	if (elems[0].tagName=='foreignobject')elems = [$(elems).find('textarea')[0]]
...
this.getArrowDecider = function(){
	var dStr = $(this.getSelectedElems()[0]).attr('d').indexOf(',')>=0?$(this.getSelectedElems()[0]).attr('d').replace(/\,/g,' '):$(this.getSelectedElems()[0]).attr('d')
	var dArr = dStr.toLowerCase().split('l'),
		point_s = {},point_e = {};
	point_s.cx = dArr[0].substr(1).trim().split(' ')[0].trim();
	point_s.cy = dArr[0].substr(1).trim().split(' ')[1].trim();
	point_e.cx = dArr[5].trim().split(' ')[0].trim();
	point_e.cy = dArr[5].trim().split(' ')[1].trim();
	return{point_s:point_s,point_e:point_e}
}
this.resolveIE = function(doc){
	var nodesToResolve = $(doc).find('foreignObject'),
		ns = 'http://www.w3.org/2000/svg';
	$.each(nodesToResolve, function() {
		var attrs = $(this)[0].attributes,
			updateNode = document.createElementNS('http://www.w3.org/2000/svg','rect');
		for(var k=0;k<attrs.length;k++){
			if(attrs[k].name=='xlink:href'){updateNode.href.baseVal = attrs[k].value;continue;}
			if(attrs[k].name.toLowerCase()=='requiredfeatures'){continue;}
			
			updateNode.setAttribute(attrs[k].name=="preserveaspectratio"?"preserveAspectRatio":attrs[k].name,attrs[k].value);
			
		}
		$(updateNode).attr({'fill':'none','class':"text-holder","style":"pointer-events:all"})
//		foreignObjTransfer.toHtml($(updateNode),$($(this).html()));
		foreignObjTransfer.toHtml($(updateNode),$($(this).children()[0].outerHTML));
		$(this).replaceWith(updateNode)
	});
}
this.updateColorGuide = function(){
	var color,selected = this.getSelectedElems(),
		curMode = this.getMode();
	if(curMode=='select'&&selected.length>1&&selected[1]!=null){return;}
	color=$(selected[0]).attr('fill')!='none'?$(selected[0]).attr('fill'):$(selected[0]).attr('stroke')!='none'?$(selected[0]).attr('stroke'):$(selected[0]).find('textarea').css('color')
	if(!!color)this.setColorGuide(color.replace('#',''));
}
this.setColorGuide= function (color){
...
this.setFileBac =function(url){
	var img = $('<img>');
	img.attr({src:url});
	img.on('load',function(){resizeHandler(img[0].offsetWidth,img[0].offsetHeight)})
	$('#file_bac').empty().append(img);
}
this.fontfamilyMap = {'microsoft yahei':'微软雅黑','fantasy':'fantasy','simsun':'宋体','arial':'arial','cursive':'cursive'}
this.isIE =  svgedit.browser.isIE()||((!!window.ActiveXObject || "ActiveXObject" in window));
}
