...
(function() {
	...
	document.getElementById('whiteBoradCover').addEventListener("touchstart", touchHandler, true);
  document.getElementById('whiteBoradCover').addEventListener("touchmove", touchHandler, true);
  document.getElementById('whiteBoradCover').addEventListener("touchend", touchHandler, true);
  document.getElementById('whiteBoradCover').addEventListener("touchcancel", touchHandler, true);
...
//				dimensions: [640,480],
				...
				initStroke: {
					width: 5,
					color: '0000ff',  // solid black
					opacity: 1
				},
				initOpacity: 1,
//				imgPath: 'images/',
				imgPath: 'message/whiteboard/images/',
				langPath: 'locale/',
//				extPath: 'extensions/',
				extPath: 'message/whiteboard/extensions/',
				jGraduatePath: 'jgraduate/images/',
//				extensions: ['ext-markers.js','ext-connector.js', 'ext-eyedropper.js', 'ext-shapes.js', 'ext-imagelib.js','ext-grid.js'],
				extensions: ['ext-markers.js', 'ext-eyedropper.js', 'ext-shapes.js', 'ext-imagelib.js','ext-grid.js','IMExtension.js'], //imcustomize!!!
				initTool: 'select',
				wireframe: false,
				colorPickerCSS: null,
				gridSnapping: false,
...
//						var fileName = location.search.split('?')[1].split('fileId=')[1].split('&')[0];
						window.temVar = Editor;
//						if($('.whbd-container').hasClass('fresh-whbd')){
//							Editor.loadFromString('<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="980" height="800"><g><title>Layer 1</title></g></svg>');
//						}
						...
						/*$.ajax({
							...
							success:function(data){
								Editor.loadFromString(""+decodeURIComponent(data));
								window.temVar = Editor;
							},
...
//				Editor.putLocale(null, good_langs);
			...
			$.svgIcons(curConfig.imgPath + 'svg_edit_icons.svg', {
				...
				placement: {
					...
					"#tool_stroke_width,#tool_stroke_width_show":"tool_stroke_width",
					"#tool_zoom_in":"zoom_in",//imcustomize!!!
...
//					'.flyout_arrow_horiz':'arrow_right', //工具栏三角
...
//						$(this).parent().prepend(this);//imcustomize!!!
					});
...
// 					}
					...
					setTimeout(function() {
						$('.flyout_arrow_horiz:empty').each(function() {
							//$(this).append($.getSvgIcon('arrow_right').width(5).height(5));//工具栏三角
						});
					}, 1);
				}
			});
			...
			var setupFlyouts = function(holders) {
				$.each(holders, function(hold_sel, btn_opts) {
					...
					shower.mousedown(function(evt) {
						...
						timer = setTimeout(function() {//return false;
...
//							holder.data('shown_popop',true);
							...
							$(window).width()>=660?
									holder.css({bottom:0,top:'auto',left:$('#tools_left').position().left+4+l-36}).show().animate({
										//left: ya
										bottom:$('#tools_left').height(),
									}, 150):
										holder.css({top:($('#tools_left').position().top+pos.top),left:'auto',right:0}).show().animate({
										//left: ya
										right: 34,
									}, 150);
									holder.data("shown_popop", true);
						...
						evt.preventDefault();
					}).mouseup(function(evt) {return false;//imcustomize!!! do not fold the flyout menu after a click on the button.
						clearTimeout(timer);
...
// 					}
					...
					if(panels[el_name]) {
						...
						if(el_name == 'text') {
							window.curTxtId = elem.id;//imcustomize!!!
			...
			if($.pref('bkgd_color')) {
				setBackground('none', $.pref('bkgd_url'));
...
//				setBackground(defaultPrefs.bkgd_color, $.pref('bkgd_url'));
			...
			$('#text_area').on('blur',function(){
				svgCanvas.setTextAreaContent(this.value);
...
// 					console.log('cm', $(evt.target).closest('.contextMenu'));
			...
			*/
			...
			var clickSelect = function() {
				if (toolButtonClick('#tool_select,.im-tool-select')) {
//					svgCanvas.clearSelection();
					svgCanvas.setMode('select');
					$('#styleoverrides').text('#svgcanvas svg *{cursor:move;pointer-events:all}, #svgcanvas svg{cursor:default}');
				}
			...
			var clickFHPath = function() {
				if (toolButtonClick('#tool_fhpath,.im-tool-pencil')) {
			...
			var clickFocus = function() {
				if (toolButtonClick('.im-tool-focus')) {
					svgCanvas.setMode('focus');
				}
			};
			...
			var clickArrow = function(){
				if (toolButtonClick('.im-tool-arrow')) {
//					window.strokeWidth = svgCanvas.getStrokeWidth();
					svgCanvas.clearSelection();
					svgCanvas.setMode('arrow');
					setFill("arrow");
					svgCanvas.arrMode = true;
				}
			};
			var clickBaloon = function(){
				if (toolButtonClick('.im-tool-baloon')) {
					svgCanvas.setMode('baloon');
				}
			};
			var clickTriangle = function(){
				if (toolButtonClick('.im-tool-triangle')) {
					svgCanvas.setMode('triangle');
				}
			};
			...
			var clickImage = function(){
				if (toolButtonClick('#tool_image,.im-tool-image')) {
//					svgCanvas.setMode('image');
					$("#tool_import input").trigger('click');//imcustomize!!!
				}
			};
			...
			var clickText = function(){
				if (toolButtonClick('#tool_text,.im-tool-text')) {
					svgCanvas.clearSelection();
					svgCanvas.setMode('text');
					setFill('text');
				}
			};
			var clickTextArea = function(e){
//				if($(svgCanvas.getSelectedElems()[0]).find('textarea').length>0){
//					svgCanvas.textareaActions.mouseUp();
//				}
				if (toolButtonClick('.im-tool-textarea')) {
					svgCanvas.setMode('textarea');
					setFill('text');
					svgCanvas.baloonType = $(e.target).closest('.im-baloons-btn').length>0?$('.im-baloons-btn li').index($(this))+'':null;
				}
			};
			...
			var clickUndo = function(){
				if (undoMgr.getUndoStackSize() > 1) {
					...
					console.log("<><><><><>",undoMgr.undoStack[undoMgr.undoStackPointer]);
					changeState(undoMgr.undoStack[undoMgr.undoStackPointer],1);//imcustomize!!!
			...
			var clickRedo = function(){
				if (undoMgr.getRedoStackSize() > 0) {
					...
					changeState(undoMgr.undoStack[undoMgr.undoStackPointer-1],0);//imcustomize!!!
...
// 					}
			...
			function promptImgURL() {return false;//imcustomize!!
			...
			(function() {
				...
				$.each(toolnames, function(i) {
					all_tools += '#tool_' + toolnames[i] + (i!=toolnames.length-1?',':'');
...
//			$(window).bind('load resize', centerCanvas); //imcustomize!!!
		...
		// 	}
			...
			var Actions = function() {
				...
				var tool_buttons = [
					{sel:'#tool_select,.im-tool-select', fn: clickSelect, evt: 'click', key: ['V', true]},
					{sel:'#tool_fhpath,.im-tool-pencil', fn: clickFHPath, evt: 'click', key: ['Q', true]},
//					{sel:'#tool_line', fn: clickLine, evt: 'click', key: ['L', true]},
					{sel:'#tool_line', fn: clickLine, evt: 'click', key: ['L', true]},
					{sel:'.im-tool-focus', fn: clickFocus, evt: 'click'},
//					{sel:'.im-tool-line', fn: clickLine, evt: 'click', key: ['L', true]},
					{sel:'.im-tool-line', fn: clickLine, evt: 'click'},
//					{sel:'#tool_rect,.im-tool-rect', fn: clickRect, evt: 'mouseup', key: ['R', true], parent: '#tools_rect', icon: 'rect'},
					{sel:'#tool_rect,.im-tool-rect', fn: clickRect, evt: 'mouseup', key: ['R', true], icon: 'rect'},
...
//					{sel:'#tool_ellipse', fn: clickEllipse, evt: 'mouseup', key: ['E', true], parent: '#tools_ellipse', icon: 'ellipse'},
					{sel:'#tool_ellipse,.im-tool-ellipse', fn: clickEllipse, evt: 'mouseup', key: ['E', true], icon: 'ellipse'},
					...
					{sel:'#tool_text,.im-tool-text', fn: clickText, evt: 'click', key: ['T', true]},
					{sel:'.im-tool-textarea,.im-baloon-textarea', fn: clickTextArea, evt: 'click'},
					{sel:'.im-tool-arrow', fn: clickArrow, evt: 'click'},
					{sel:'.im-tool-baloon', fn: clickBaloon, evt: 'click'},
					{sel:'.im-tool-triangle', fn: clickTriangle, evt: 'click'},
					{sel:'#tool_image,.im-tool-image', fn: clickImage, evt: 'mouseup'},
					{sel:'#tool_zoom', fn: clickZoom, evt: 'mouseup', key: ['Z', true]},
//					{sel:'#tool_save', fn: function() { editingsource?saveSourceEditor():clickSave()}, evt: 'mouseup', key: ['S', true]},//cancel hotkey s imcustomize 
...
//					{sel:'#tool_clone,#tool_clone_multi', fn: clickClone, evt: 'click', key: ['D', true]},
					{sel:'#tool_clear', fn: clickClear, evt: 'mouseup'},//cancel hotkey n imcustomize
					{sel:'#tool_path', fn: clickPath, evt: 'click'},//cancel hotkey p imcustomize
					{sel:'#tool_source', fn: showSourceEditor, evt: 'click'},//cancel hotkey u imcustomize
					{sel:'#tool_wireframe', fn: clickWireframe, evt: 'click'},//cancel hotkey f imcustomize
					{sel:'#tool_clone,#tool_clone_multi', fn: clickClone, evt: 'click'},//cancel hotkey d imcustomize
					{sel:'#sidepanel_handle', fn: toggleSidePanel},//cancel hotkey x imcustomize
					...
					{sel:'#tool_delete,.im-tool-eraser,#tool_delete_multi', fn: deleteSelected, evt: 'click', key: ['del/backspace', true]},
					...
					{sel:'#tool_undo,.im-tool-undo', fn: clickUndo, evt: 'click', key: ['Z', true]},
					{sel:'#tool_redo,.im-tool-redo', fn: clickRedo, evt: 'click', key: ['Y', true]},
		...
		// 			{sel:'#tools_ellipse_show', fn: clickEllipse, evt: 'click'},
			...
			$("#workarea").contextMenu({},function(){return;})
...
//			});
			...
			/*window.onbeforeunload = function() {alert()
				...
				if ('localStorage' in window) {alert()
					var name = 'svgedit-' + Editor.curConfig.canvasName;
						...
						if(file.type.indexOf("image") != -1) {
							...
							//bitmap handling
								...
								reader.onloadend = function(e) {
									...
									insertNewImage = function(img_width, img_height,src){
...
//										svgCanvas.setHref(newImage, e.target.result);
										...
										svgCanvas.setHref(newImage, src);
...
//										updateContextPanel();
									...
									YYIMChat.sendPic({
										...
										success:function(data){
											var J = 100,
												M = 100,
												Q = new Image;
											Q.src = data.body.content.path;
											Q.style.opacity = 0;
											Q.onload = function() {
												J = Q.offsetWidth;
												M = Q.offsetHeight>$('#svgcanvas').height()?$('#svgcanvas').height():Q.offsetHeight;
				...
				var img_import = $('<input type="file" id="add_pic_to_whbd" accept="image/*">').change(import_image);
...
// 			});
		...
		Editor.loadSnapshots=function(){
			var transformedPics = [],i,appendStr='',
				files = window.currentWhiteBoard.files;
			if(files&&files.length>0){
					for(i=0;i<files.length;i++){
						appendStr+='<li><img src="'+YYIMChat.getFileUrl(files[i],'middle','transform')+'"/>li'
//						appendStr+='<li><img src="'+getSnapshot(null,files[i])+'"/>li'
					}
			}else{
