...
SNSApplication.setCookie=function(a,b,c){var d=arguments,e=arguments.length;c=2<e?d[2]:null;var g=3<e?d[3]:null,s=4<e?d[4]:null,d=5<e?d[5]:!1;document.cookie=a+"="+escape(b)+(null==c?"":"; expires="+c.toGMTString())+(g?"/":"; path="+g)+(null==s?"":"; domain="+s)+(!0==d?"; secure":"")};SNSApplication.getCookie=function(a){for(var b=document.cookie.split(";"),c=0;c<b.length;c++){var d=b[c].split("="),e=d[0].replace(/(^\s*)|(\s$)/g,"");if(a==e)return unescape(d[1])}return null};
...
!1))};SNSHandler.prototype.onUserBind=function(a,b){var c=SNSApplication.getInstance().getUser();c.id=a;c.name=a;c.resource=b};SNSHandler.prototype.onPresence=function(a){YYIMChat.log("\u597d\u53cb"+a.from+"\u7684\u72b6\u6001\u6539\u53d8",3,a);var b=a.from,c=new SNSPresence(a),d=SNSApplication.getInstance().getUser();b==d.id&&a.resource==d.resource?(d.setPresence(c,!1),SNSApplication.setCookie("presence.status",JSON.stringify({user:d.id,status:c.status}),new Date((new Date).getTime()+12096E5))):d.getOrCreateRoster(b).setPresence(c)};
...
SNSHandler.prototype.onMessage=function(a){var b=SNSApplication.getInstance().getMessageInBox(),c=new SNSInMessage;SNSApplication.getInstance().getUser();if(a.type==SNS_CHAT_TYPE.GROUP_CHAT){if(a.from.roster==SNSApplication.getInstance().getUser().id)return;c.initRoomMessage(a)}else a.type==SNS_CHAT_TYPE.CHAT||a.type==SNS_CHAT_TYPE.PUB_ACCOUNT?c.initRosterMessage(a):(a.type=SNS_CHAT_TYPE.DEVICE)&&c.initDeviceMessage(a);b.filter.doFilter(c);b.addToUnreadMessage(c);SNSHandler.getInstance().afterInitialized?
(SNSApplication.getInstance().getUser().recentList.addNew(c),SNSApplication.getInstance().getGlobalEventManager().trigger(SNS_EVENT_SUBJECT.ON_MESSAGE_IN,[{message:c}])):b.addReceivedMessage(c)};SNSHandler.prototype.onReceipts=function(a){2==a.state&&YYIMChat.getUserID()};SNSHandler.prototype.onTextMessage=function(a){SNSHandler.getInstance().onMessage(a)};SNSHandler.prototype.onPictureMessage=function(a){SNSHandler.getInstance().onMessage(a)};SNSHandler.prototype.onFileMessage=function(a){SNSHandler.getInstance().onMessage(a)};
SNSHandler.prototype.onDeviceMessage=function(a){SNSHandler.getInstance().onMessage(a)};SNSHandler.prototype.onShareMessage=function(a){SNSHandler.getInstance().onMessage(a)};SNSHandler.prototype.onSystemMessage=function(a){SNSHandler.getInstance().onMessage(a)};SNSHandler.prototype.onPublicMessage=function(a){SNSHandler.getInstance().onMessage(a)};SNSHandler.prototype.onGroupUpdate=function(a){YYIMChat.log("\u7fa4\u7ec4\u4fe1\u606f\u66f4\u65b0",3,a)};
SNSHandler.prototype.onTransferGroupOwner=function(a){YYIMChat.log("\u7fa4\u4e3b\u66f4\u65b0",3,a)};SNSHandler.prototype.onKickedOutGroup=function(a){YYIMChat.log("\u88ab\u7fa4\u7ec4\u8e22\u51fa",3,a)};SNSHandler.prototype.onAudoMessage=function(a){YYIMChat.log("\u97f3\u9891\u6d88\u606f",3,a)};SNSHandler.prototype.onLocationMessage=function(a){YYIMChat.log("\u4f4d\u7f6e\u6d88\u606f",3,a)};SNSHandler.prototype.onWhiteBoardMessage=function(a){YYIMChat.log("\u767d\u677f\u6d88\u606f",3,a)};
...
SNSMessageInBox.prototype.addToUnreadMessage=function(a){var b=a.getRosterOrChatRoom().getID(),c=this.unreadMessageList.get(b);c||(c=[],this.unreadMessageList.add(b,c));c.push(a)};SNSMessageInBox.prototype.addReceivedMessage=function(a){this.receivedMessageList.push(a)};SNSMessageInBox.prototype.removeFromUnreadMessage=function(a,b){this.popUnreadMessageByRoster(b.getRosterOrChatRoom())};
...
var SNSMessage=function(a){this.id=Math.uuid();this.subject=this.thread=this.roster=this.chatroom=this.from=this.to=null;this.type=SNS_MESSAGE_TYPE.CHAT;this.body={expression:0,contentType:SNS_MESSAGE_CONTENT_TYPE.TEXT,content:null,dateline:(new Date).getTime()}};SNSMessage.prototype.getBodyHtml=function(){return!this.html||this.html.isEmpty()?this.body.content:this.html};
SNSMessage.prototype.getRoster=function(){if(this.to&&this.to instanceof SNSRoster)return this.to;if(this.from&&this.from instanceof SNSRoster)return this.from;YYIMChat.log("snsMessage.getRoster",1,this.to,this.from)};SNSMessage.prototype.getRosterOrChatRoom=function(){return this.type==SNS_MESSAGE_TYPE.CHAT||this.type==SNS_MESSAGE_TYPE.PUBACCOUNT?this.getRoster():this.type==SNS_MESSAGE_TYPE.GROUPCHAT?this.chatroom:this.from};
SNSMessage.prototype.setThread=function(a){a&&a.notEmpty()&&(this.thread=a)};SNSMessage.prototype.setSubject=function(a){a&&a.notEmpty()&&(this.subject=a)};
SNSMessage.prototype.getHumanizeDateString=function(){function a(a){return 9<a?a:"0"+a}var b=this.body.dateline;b||(dataline=new Date);if("number"==typeof b){var c=new Date;c.setTime(b);b=c}if(b&&b instanceof Date){var d=b.getYear(),e=a(b.getMonth()+1),g=a(b.getDate()),s=a(b.getHours()),b=a(b.getMinutes()),c=new Date;return d!=c.getYear()?d+"-"+e+"-"+g+" "+s+":"+b:e!=c.getMonth()||g!=c.getDate()?e+"-"+g+" "+s+":"+b:12>s?SNS_I18N.am+s+":"+b:SNS_I18N.pm+s+":"+b}YYIMChat.log("snsMessage.getHumanizeDateString:invalid dateline",
...
SNSInMessage.prototype.initRoomMessage=function(a){var b=SNSApplication.getInstance().getUser();this.id=a.id||this.id;this.type=SNS_CHAT_TYPE.GROUP_CHAT;this.from=b.getOrCreateRoster(a.from.roster);this.chatroom=b.chatRoomList.getChatRoom(a.from.room);this.body=Object.clone(a.data);this.body.style=this.decodeMessageStyle(a.data.style)};
SNSInMessage.prototype.initRosterMessage=function(a){this.id=a.id||this.id;this.type=SNS_CHAT_TYPE.CHAT;this.from=SNSApplication.getInstance().getUser().getOrCreateRoster(a.from,a.resource);a.resource&&(this.resource=a.resource);this.body=Object.clone(a.data);this.body.style=this.decodeMessageStyle(a.data.style)};
...
SNSUser.prototype.rosterListHandler=function(a){if((a=JSON.parse(a))&&!(0>=a.length)){for(var b=0;b<a.length;b++){var c=new SNSRoster(a[b].id);c.name=a[b].name;c.ask=a[b].ask;c.photoUrl=a[b].photo;c.subscription=a[b].subscription;if(c.subscription==SNS_SUBSCRIBE.BOTH){var d=a[b].group;if(d&&0!=d.length)for(var e=0;e<d.length;e++){var g=this.groupList.getGroup(d[e]);g||(g=this.groupList.addGroup(d[e]));c.addToGroup(g,!0)}else c.addToGroup(this.groupList.getGroup(SNSConfig.GROUP.GROUP_NONE),!0);this.addRoster(c)}}this.requestRosterVCards()}};
SNSUser.prototype.addRoster=function(a,b){var c=this.getRoster(a);if(b||c!=a)this.rosterList.add(a),SNSApplication.getInstance().getGlobalEventManager().trigger(SNS_EVENT_SUBJECT.ON_ADD_ROSTER,[{newValue:a,oldValue:c}])};SNSUser.prototype.getOrCreateRoster=function(a,b){var c=this.getRoster(a);c||(c=new SNSRoster(a),b&&(c.resource=b),this.addRoster(c));return c};SNSUser.prototype.getRoster=function(a){var b=this.deviceList.get(a);return b?b:this.systemRoster.getID()==a?this.systemRoster:this.rosterList.get(a)};
SNSUser.prototype.getRosterOrChatRoom=function(a){return this.getRoster(a)?this.getRoster(a):this.chatRoomList.getChatRoom(a)};SNSUser.prototype.getRoom=function(a){return this.chatRoomList.getChatRoom(a)};
...
SNSRecentList.prototype.addNew=function(a){var b=a.getRosterOrChatRoom().id;if(!this.list[0]||this.list[0].id!=b){a=this.list.unshift({id:b,message:a.body.content});for(var c=1;c<a;c++)if(this.list[c].id==b){this.list.splice(c,1);break}this.list.length>SNSConfig.RECENT.MAX_SIZE&&this.list.pop();SNSApplication.getInstance().getGlobalEventManager().trigger(SNS_EVENT_SUBJECT.ADD_TO_RENCENT,[])}};
...
SNSRosterList.prototype.getAvailableRosters=function(){var a=[],b;for(b in this.list)b.presence.status==SNS_STATUS.AVAILABLE&&a.push(b);return a};SNSRosterList.prototype.getRostersBySubscription=function(a,b,c){var d=[],e;for(e in this.list)for(var g=e.subscription,s=0;s<arguments.length;s++)argumetns[s]==g&&d.push(e);return d};var SNSSystemRoster=function(){this.name="\u7cfb\u7edf\u6d88\u606f";this.id="system";this.show="systemroster";this.messageList=[];this.photoUrl=SNSConfig.SYSTEM_ROSTER.DEFAULT_AVATAR};
...
(function(a){function b(a,b){var c=(a&65535)+(b&65535);return(a>>16)+(b>>16)+(c>>16)<<16|c&65535}function c(a,c,d,e,g,s){a=b(b(c,a),b(e,s));return b(a<<g|a>>>32-g,d)}function d(a,b,d,e,g,s,u){return c(b&d|~b&e,a,b,g,s,u)}function e(a,b,d,e,g,s,u){return c(b&e|d&~e,a,b,g,s,u)}function g(a,b,d,e,g,s,u){return c(d^(b|~e),a,b,g,s,u)}function s(a,k){a[k>>5]|=128<<k%32;a[(k+64>>>9<<4)+14]=k;var p,s,u,t,O,f=1732584193,m=-271733879,l=-1732584194,h=271733878;for(p=0;p<a.length;p+=16)s=f,u=m,t=l,O=h,f=d(f,
m,l,h,a[p],7,-680876936),h=d(h,f,m,l,a[p+1],12,-389564586),l=d(l,h,f,m,a[p+2],17,606105819),m=d(m,l,h,f,a[p+3],22,-1044525330),f=d(f,m,l,h,a[p+4],7,-176418897),h=d(h,f,m,l,a[p+5],12,1200080426),l=d(l,h,f,m,a[p+6],17,-1473231341),m=d(m,l,h,f,a[p+7],22,-45705983),f=d(f,m,l,h,a[p+8],7,1770035416),h=d(h,f,m,l,a[p+9],12,-1958414417),l=d(l,h,f,m,a[p+10],17,-42063),m=d(m,l,h,f,a[p+11],22,-1990404162),f=d(f,m,l,h,a[p+12],7,1804603682),h=d(h,f,m,l,a[p+13],12,-40341101),l=d(l,h,f,m,a[p+14],17,-1502002290),
m=d(m,l,h,f,a[p+15],22,1236535329),f=e(f,m,l,h,a[p+1],5,-165796510),h=e(h,f,m,l,a[p+6],9,-1069501632),l=e(l,h,f,m,a[p+11],14,643717713),m=e(m,l,h,f,a[p],20,-373897302),f=e(f,m,l,h,a[p+5],5,-701558691),h=e(h,f,m,l,a[p+10],9,38016083),l=e(l,h,f,m,a[p+15],14,-660478335),m=e(m,l,h,f,a[p+4],20,-405537848),f=e(f,m,l,h,a[p+9],5,568446438),h=e(h,f,m,l,a[p+14],9,-1019803690),l=e(l,h,f,m,a[p+3],14,-187363961),m=e(m,l,h,f,a[p+8],20,1163531501),f=e(f,m,l,h,a[p+13],5,-1444681467),h=e(h,f,m,l,a[p+2],9,-51403784),
l=e(l,h,f,m,a[p+7],14,1735328473),m=e(m,l,h,f,a[p+12],20,-1926607734),f=c(m^l^h,f,m,a[p+5],4,-378558),h=c(f^m^l,h,f,a[p+8],11,-2022574463),l=c(h^f^m,l,h,a[p+11],16,1839030562),m=c(l^h^f,m,l,a[p+14],23,-35309556),f=c(m^l^h,f,m,a[p+1],4,-1530992060),h=c(f^m^l,h,f,a[p+4],11,1272893353),l=c(h^f^m,l,h,a[p+7],16,-155497632),m=c(l^h^f,m,l,a[p+10],23,-1094730640),f=c(m^l^h,f,m,a[p+13],4,681279174),h=c(f^m^l,h,f,a[p],11,-358537222),l=c(h^f^m,l,h,a[p+3],16,-722521979),m=c(l^h^f,m,l,a[p+6],23,76029189),f=c(m^
l^h,f,m,a[p+9],4,-640364487),h=c(f^m^l,h,f,a[p+12],11,-421815835),l=c(h^f^m,l,h,a[p+15],16,530742520),m=c(l^h^f,m,l,a[p+2],23,-995338651),f=g(f,m,l,h,a[p],6,-198630844),h=g(h,f,m,l,a[p+7],10,1126891415),l=g(l,h,f,m,a[p+14],15,-1416354905),m=g(m,l,h,f,a[p+5],21,-57434055),f=g(f,m,l,h,a[p+12],6,1700485571),h=g(h,f,m,l,a[p+3],10,-1894986606),l=g(l,h,f,m,a[p+10],15,-1051523),m=g(m,l,h,f,a[p+1],21,-2054922799),f=g(f,m,l,h,a[p+8],6,1873313359),h=g(h,f,m,l,a[p+15],10,-30611744),l=g(l,h,f,m,a[p+6],15,-1560198380),
m=g(m,l,h,f,a[p+13],21,1309151649),f=g(f,m,l,h,a[p+4],6,-145523070),h=g(h,f,m,l,a[p+11],10,-1120210379),l=g(l,h,f,m,a[p+2],15,718787259),m=g(m,l,h,f,a[p+9],21,-343485551),f=b(f,s),m=b(m,u),l=b(l,t),h=b(h,O);return[f,m,l,h]}function u(a){var b,c="";for(b=0;b<32*a.length;b+=8)c+=String.fromCharCode(a[b>>5]>>>b%32&255);return c}function t(a){var b,c=[];c[(a.length>>2)-1]=void 0;for(b=0;b<c.length;b+=1)c[b]=0;for(b=0;b<8*a.length;b+=8)c[b>>5]|=(a.charCodeAt(b/8)&255)<<b%32;return c}function v(a){return u(s(t(a),
8*a.length))}function P(a){var b="",c,d;for(d=0;d<a.length;d+=1)c=a.charCodeAt(d),b+="0123456789abcdef".charAt(c>>>4&15)+"0123456789abcdef".charAt(c&15);return b}function V(a,b){var c=unescape(encodeURIComponent(a)),d=unescape(encodeURIComponent(b)),e=t(c),g=[],O=[];g[15]=O[15]=void 0;16<e.length&&(e=s(e,8*c.length));for(c=0;16>c;c+=1)g[c]=e[c]^909522486,O[c]=e[c]^1549556828;d=s(g.concat(t(d)),512+8*d.length);return u(s(O.concat(d),640))}a.md5=function(a,b,c){return b?c?V(b,a):P(V(b,a)):c?v(unescape(encodeURIComponent(a))):
P(v(unescape(encodeURIComponent(a))))}})("function"===typeof jQuery?jQuery:this);
(function(a){var b,c,d,e,g,s,u={},t={},v={},P={treeId:"",treeObj:null,view:{addDiyDom:null,autoCancelSelected:!0,dblClickExpand:!0,expandSpeed:"fast",fontCss:{},nameIsHTML:!1,selectedMulti:!0,showIcon:!0,showLine:!0,showTitle:!0,txtSelectedEnable:!1},data:{key:{children:"children",name:"name",title:"",url:"url"},simpleData:{enable:!1,idKey:"id",pIdKey:"pId",rootPId:null},keep:{parent:!1,leaf:!1}},async:{enable:!1,contentType:"application/x-www-form-urlencoded",type:"post",dataType:"text",url:"",autoParam:[],
otherParam:[],dataFilter:null},callback:{beforeAsync:null,beforeClick:null,beforeDblClick:null,beforeRightClick:null,beforeMouseDown:null,beforeMouseUp:null,beforeExpand:null,beforeCollapse:null,beforeRemove:null,onAsyncError:null,onAsyncSuccess:null,onNodeCreated:null,onClick:null,onDblClick:null,onRightClick:null,onMouseDown:null,onMouseUp:null,onExpand:null,onCollapse:null,onRemove:null}},V=[function(a){var b=a.treeObj,c=q.event;b.bind(c.NODECREATED,function(b,c,A){n.apply(a.callback.onNodeCreated,
...
q.event;a.treeObj.unbind(b.NODECREATED).unbind(b.CLICK).unbind(b.EXPAND).unbind(b.COLLAPSE).unbind(b.ASYNC_SUCCESS).unbind(b.ASYNC_ERROR).unbind(b.REMOVE)}],k=[function(a){var b=h.getCache(a);b||(b={},h.setCache(a,b));b.nodes=[];b.doms=[]}],p=[function(a,b,c,d,e,J){if(c){var f=h.getRoot(a),g=a.data.key.children;c.level=b;c.tId=a.treeId+"_"+ ++f.zId;c.parentTId=d?d.tId:null;c.open="string"==typeof c.open?n.eqs(c.open,"true"):!!c.open;c[g]&&0<c[g].length?(c.isParent=!0,c.zAsync=!0):(c.isParent="string"==
typeof c.isParent?n.eqs(c.isParent,"true"):!!c.isParent,c.open=c.isParent&&!a.async.enable?c.open:!1,c.zAsync=!c.isParent);c.isFirstNode=e;c.isLastNode=J;c.getParentNode=function(){return h.getNodeCache(a,c.parentTId)};c.getPreNode=function(){return h.getPreNode(a,c)};c.getNextNode=function(){return h.getNextNode(a,c)};c.isAjaxing=!1;h.fixPIdKeyValue(a,c)}}],E=[function(a){var A=a.target,H=h.getSetting(a.data.treeId),K="",F=null,J="",f="",m=null,l=null,k=null;if(n.eqs(a.type,"mousedown"))f="mousedown";
else if(n.eqs(a.type,"mouseup"))f="mouseup";else if(n.eqs(a.type,"contextmenu"))f="contextmenu";else if(n.eqs(a.type,"click"))if(n.eqs(A.tagName,"span")&&null!==A.getAttribute("treeNode"+q.id.SWITCH))K=n.getNodeMainDom(A).id,J="switchNode";else{if(k=n.getMDom(H,A,[{tagName:"a",attrName:"treeNode"+q.id.A}]))K=n.getNodeMainDom(k).id,J="clickNode"}else n.eqs(a.type,"dblclick")&&(f="dblclick",k=n.getMDom(H,A,[{tagName:"a",attrName:"treeNode"+q.id.A}]))&&(K=n.getNodeMainDom(k).id,J="switchNode");0<f.length&&
0==K.length&&(k=n.getMDom(H,A,[{tagName:"a",attrName:"treeNode"+q.id.A}]))&&(K=n.getNodeMainDom(k).id);if(0<K.length)switch(F=h.getNodeCache(H,K),J){case "switchNode":F.isParent?n.eqs(a.type,"click")||n.eqs(a.type,"dblclick")&&n.apply(H.view.dblClickExpand,[H.treeId,F],H.view.dblClickExpand)?m=b:J="":J="";break;case "clickNode":m=c}switch(f){case "mousedown":l=d;break;case "mouseup":l=e;break;case "dblclick":l=g;break;case "contextmenu":l=s}return{stop:!1,node:F,nodeEventType:J,nodeEventCallback:m,
treeEventType:f,treeEventCallback:l}}],I=[function(a){var b=h.getRoot(a);b||(b={},h.setRoot(a,b));b[a.data.key.children]=[];b.expandTriggerFlag=!1;b.curSelectedList=[];b.noSelection=!0;b.createdNodes=[];b.zId=0;b._ver=(new Date).getTime()}],L=[],O=[],f=[],m=[],l=[],h={addNodeCache:function(a,b){h.getCache(a).nodes[h.getNodeCacheId(b.tId)]=b},getNodeCacheId:function(a){return a.substring(a.lastIndexOf("_")+1)},addAfterA:function(a){O.push(a)},addBeforeA:function(a){L.push(a)},addInnerAfterA:function(a){m.push(a)},
addInnerBeforeA:function(a){f.push(a)},addInitBind:function(a){V.push(a)},addInitUnBind:function(a){C.push(a)},addInitCache:function(a){k.push(a)},addInitNode:function(a){p.push(a)},addInitProxy:function(a,b){b?E.splice(0,0,a):E.push(a)},addInitRoot:function(a){I.push(a)},addNodesData:function(a,b,c){var d=a.data.key.children;b[d]||(b[d]=[]);0<b[d].length&&(b[d][b[d].length-1].isLastNode=!1,r.setNodeLineIcos(a,b[d][b[d].length-1]));b.isParent=!0;b[d]=b[d].concat(c)},addSelectedNode:function(a,b){var c=
h.getRoot(a);h.isSelectedNode(a,b)||c.curSelectedList.push(b)},addCreatedNode:function(a,b){(a.callback.onNodeCreated||a.view.addDiyDom)&&h.getRoot(a).createdNodes.push(b)},addZTreeTools:function(a){l.push(a)},exSetting:function(b){a.extend(!0,P,b)},fixPIdKeyValue:function(a,b){a.data.simpleData.enable&&(b[a.data.simpleData.pIdKey]=b.parentTId?b.getParentNode()[a.data.simpleData.idKey]:a.data.simpleData.rootPId)},getAfterA:function(a,b,c){for(var d=0,e=O.length;d<e;d++)O[d].apply(this,arguments)},
getBeforeA:function(a,b,c){for(var d=0,e=L.length;d<e;d++)L[d].apply(this,arguments)},getInnerAfterA:function(a,b,c){for(var d=0,e=m.length;d<e;d++)m[d].apply(this,arguments)},getInnerBeforeA:function(a,b,c){for(var d=0,e=f.length;d<e;d++)f[d].apply(this,arguments)},getCache:function(a){return v[a.treeId]},getNextNode:function(a,b){if(!b)return null;for(var c=a.data.key.children,d=b.parentTId?b.getParentNode():h.getRoot(a),e=0,f=d[c].length-1;e<=f;e++)if(d[c][e]===b)return e==f?null:d[c][e+1];return null},
getNodeByParam:function(a,b,c,d){if(!b||!c)return null;for(var e=a.data.key.children,f=0,g=b.length;f<g;f++){if(b[f][c]==d)return b[f];var m=h.getNodeByParam(a,b[f][e],c,d);if(m)return m}return null},getNodeCache:function(a,b){if(!b)return null;var c=v[a.treeId].nodes[h.getNodeCacheId(b)];return c?c:null},getNodeName:function(a,b){return""+b[a.data.key.name]},getNodeTitle:function(a,b){return""+b[""===a.data.key.title?a.data.key.name:a.data.key.title]},getNodes:function(a){return h.getRoot(a)[a.data.key.children]},
getNodesByParam:function(a,b,c,d){if(!b||!c)return[];for(var e=a.data.key.children,f=[],g=0,m=b.length;g<m;g++)b[g][c]==d&&f.push(b[g]),f=f.concat(h.getNodesByParam(a,b[g][e],c,d));return f},getNodesByParamFuzzy:function(a,b,c,d){if(!b||!c)return[];var e=a.data.key.children,f=[];d=d.toLowerCase();for(var g=0,m=b.length;g<m;g++)"string"==typeof b[g][c]&&-1<b[g][c].toLowerCase().indexOf(d)&&f.push(b[g]),f=f.concat(h.getNodesByParamFuzzy(a,b[g][e],c,d));return f},getNodesByFilter:function(a,b,c,d,e){if(!b)return d?
null:[];for(var f=a.data.key.children,g=d?null:[],m=0,l=b.length;m<l;m++){if(n.apply(c,[b[m],e],!1)){if(d)return b[m];g.push(b[m])}var k=h.getNodesByFilter(a,b[m][f],c,d,e);if(d&&k)return k;g=d?k:g.concat(k)}return g},getPreNode:function(a,b){if(!b)return null;for(var c=a.data.key.children,d=b.parentTId?b.getParentNode():h.getRoot(a),e=0,f=d[c].length;e<f;e++)if(d[c][e]===b)return 0==e?null:d[c][e-1];return null},getRoot:function(a){return a?t[a.treeId]:null},getRoots:function(){return t},getSetting:function(a){return u[a]},
getSettings:function(){return u},getZTreeTools:function(a){return(a=this.getRoot(this.getSetting(a)))?a.treeTools:null},initCache:function(a){for(var b=0,c=k.length;b<c;b++)k[b].apply(this,arguments)},initNode:function(a,b,c,d,e,h){for(var f=0,g=p.length;f<g;f++)p[f].apply(this,arguments)},initRoot:function(a){for(var b=0,c=I.length;b<c;b++)I[b].apply(this,arguments)},isSelectedNode:function(a,b){for(var c=h.getRoot(a),d=0,e=c.curSelectedList.length;d<e;d++)if(b===c.curSelectedList[d])return!0;return!1},
...
doProxy:function(a){for(var b=[],c=0,d=E.length;c<d;c++){var e=E[c].apply(this,arguments);b.push(e);if(e.stop)break}return b},proxy:function(a){var b=h.getSetting(a.data.treeId);if(!n.uCanDo(b,a))return!0;for(var b=G.doProxy(a),c=!0,d=0,e=b.length;d<e;d++){var f=b[d];f.nodeEventCallback&&(c=f.nodeEventCallback.apply(f,[a,f.node])&&c);f.treeEventCallback&&(c=f.treeEventCallback.apply(f,[a,f.node])&&c)}return c}};b=function(a,b){var c=h.getSetting(a.data.treeId);if(b.open){if(!1==n.apply(c.callback.beforeCollapse,
[c.treeId,b],!0))return!0}else if(!1==n.apply(c.callback.beforeExpand,[c.treeId,b],!0))return!0;h.getRoot(c).expandTriggerFlag=!0;r.switchNode(c,b);return!0};c=function(a,b){var c=h.getSetting(a.data.treeId),d=c.view.autoCancelSelected&&(a.ctrlKey||a.metaKey)&&h.isSelectedNode(c,b)?0:c.view.autoCancelSelected&&(a.ctrlKey||a.metaKey)&&c.view.selectedMulti?2:1;if(!1==n.apply(c.callback.beforeClick,[c.treeId,b,d],!0))return!0;0===d?r.cancelPreSelectedNode(c,b):r.selectNode(c,b,2===d);c.treeObj.trigger(q.event.CLICK,
[a,c.treeId,b,d]);return!0};d=function(a,b){var c=h.getSetting(a.data.treeId);n.apply(c.callback.beforeMouseDown,[c.treeId,b],!0)&&n.apply(c.callback.onMouseDown,[a,c.treeId,b]);return!0};e=function(a,b){var c=h.getSetting(a.data.treeId);n.apply(c.callback.beforeMouseUp,[c.treeId,b],!0)&&n.apply(c.callback.onMouseUp,[a,c.treeId,b]);return!0};g=function(a,b){var c=h.getSetting(a.data.treeId);n.apply(c.callback.beforeDblClick,[c.treeId,b],!0)&&n.apply(c.callback.onDblClick,[a,c.treeId,b]);return!0};
s=function(a,b){var c=h.getSetting(a.data.treeId);n.apply(c.callback.beforeRightClick,[c.treeId,b],!0)&&n.apply(c.callback.onRightClick,[a,c.treeId,b]);return"function"!=typeof c.callback.onRightClick};var n={apply:function(a,b,c){return"function"==typeof a?a.apply(ja,b?b:[]):c},canAsync:function(a,b){var c=a.data.key.children;return a.async.enable&&b&&b.isParent&&!(b.zAsync||b[c]&&0<b[c].length)},clone:function(a){if(null===a)return null;var b=n.isArray(a)?[]:{},c;for(c in a)b[c]=a[c]instanceof Date?
...
c[d].tagName)&&null!==b.getAttribute(c[d].attrName))return b;b=b.parentNode}return null},getNodeMainDom:function(b){return a(b).parent("li").get(0)||a(b).parentsUntil("li").parent().get(0)},isChildOrSelf:function(b,c){return 0<a(b).closest("#"+c).length},uCanDo:function(){return!0}},r={addNodes:function(a,b,c,d){if(!a.data.keep.leaf||!b||b.isParent)if(n.isArray(c)||(c=[c]),a.data.simpleData.enable&&(c=h.transformTozTreeFormat(a,c)),b){var e=y(b,q.id.SWITCH,a),f=y(b,q.id.ICON,a),g=y(b,q.id.UL,a);b.open||
(r.replaceSwitchClass(b,e,q.folder.CLOSE),r.replaceIcoClass(b,f,q.folder.CLOSE),b.open=!1,g.css({display:"none"}));h.addNodesData(a,b,c);r.createNodes(a,b.level+1,c,b);d||r.expandCollapseParentNode(a,b,!0)}else h.addNodesData(a,h.getRoot(a),c),r.createNodes(a,0,c,null)},appendNodes:function(a,b,c,d,e,f){if(!c)return[];for(var g=[],m=a.data.key.children,l=0,k=c.length;l<k;l++){var n=c[l];if(e){var p=(d?d:h.getRoot(a))[m].length==c.length&&0==l;h.initNode(a,b,n,d,p,l==c.length-1,f);h.addNodeCache(a,
n)}p=[];n[m]&&0<n[m].length&&(p=r.appendNodes(a,b+1,n[m],n,e,f&&n.open));f&&(r.makeDOMNodeMainBefore(g,a,n),r.makeDOMNodeLine(g,a,n),h.getBeforeA(a,n,g),r.makeDOMNodeNameBefore(g,a,n),h.getInnerBeforeA(a,n,g),r.makeDOMNodeIcon(g,a,n),h.getInnerAfterA(a,n,g),r.makeDOMNodeNameAfter(g,a,n),h.getAfterA(a,n,g),n.isParent&&n.open&&r.makeUlHtml(a,n,g,p.join("")),r.makeDOMNodeMainAfter(g,a,n),h.addCreatedNode(a,n))}return g},appendParentULDom:function(a,b){var c=[],d=y(b,a);!d.get(0)&&b.parentTId&&(r.appendParentULDom(a,
b.getParentNode()),d=y(b,a));var e=y(b,q.id.UL,a);e.get(0)&&e.remove();e=r.appendNodes(a,b.level+1,b[a.data.key.children],b,!1,!0);r.makeUlHtml(a,b,c,e.join(""));d.append(c.join(""))},asyncNode:function(b,c,d,e){var f,g;if(c&&!c.isParent)return n.apply(e),!1;if(c&&c.isAjaxing)return!1;if(!1==n.apply(b.callback.beforeAsync,[b.treeId,c],!0))return n.apply(e),!1;c&&(c.isAjaxing=!0,y(c,q.id.ICON,b).attr({style:"","class":q.className.BUTTON+" "+q.className.ICO_LOADING}));var m={};f=0;for(g=b.async.autoParam.length;c&&
f<g;f++){var l=b.async.autoParam[f].split("="),k=l;1<l.length&&(k=l[1],l=l[0]);m[k]=c[l]}if(n.isArray(b.async.otherParam))for(f=0,g=b.async.otherParam.length;f<g;f+=2)m[b.async.otherParam[f]]=b.async.otherParam[f+1];else for(var p in b.async.otherParam)m[p]=b.async.otherParam[p];var s=h.getRoot(b)._ver;a.ajax({contentType:b.async.contentType,type:b.async.type,url:n.apply(b.async.url,[b.treeId,c],b.async.url),data:m,dataType:b.async.dataType,success:function(a){if(s==h.getRoot(b)._ver){var f=[];try{f=
a&&0!=a.length?"string"==typeof a?eval("("+a+")"):a:[]}catch(g){f=a}c&&(c.isAjaxing=null,c.zAsync=!0);r.setNodeLineIcos(b,c);f&&""!==f?(f=n.apply(b.async.dataFilter,[b.treeId,c,f],f),r.addNodes(b,c,f?n.clone(f):[],!!d)):r.addNodes(b,c,[],!!d);b.treeObj.trigger(q.event.ASYNC_SUCCESS,[b.treeId,c,a]);n.apply(e)}},error:function(a,d,e){s==h.getRoot(b)._ver&&(c&&(c.isAjaxing=null),r.setNodeLineIcos(b,c),b.treeObj.trigger(q.event.ASYNC_ERROR,[b.treeId,c,a,d,e]))}});return!0},cancelPreSelectedNode:function(a,
b){for(var c=h.getRoot(a).curSelectedList,d=c.length-1;0<=d;d--)if(!b||b===c[d])if(y(c[d],q.id.A,a).removeClass(q.node.CURSELECTED),b){h.removeSelectedNode(a,b);break}b||(h.getRoot(a).curSelectedList=[])},createNodeCallback:function(a){if(a.callback.onNodeCreated||a.view.addDiyDom)for(var b=h.getRoot(a);0<b.createdNodes.length;){var c=b.createdNodes.shift();n.apply(a.view.addDiyDom,[a.treeId,c]);a.callback.onNodeCreated&&a.treeObj.trigger(q.event.NODECREATED,[a.treeId,c])}},createNodes:function(a,
b,c,d){if(c&&0!=c.length){var e=h.getRoot(a),f=a.data.key.children,f=!d||d.open||!!y(d[f][0],a).get(0);e.createdNodes=[];b=r.appendNodes(a,b,c,d,!0,f);d?(d=y(d,q.id.UL,a),d.get(0)&&d.append(b.join(""))):a.treeObj.append(b.join(""));r.createNodeCallback(a)}},destroy:function(a){a&&(h.initCache(a),h.initRoot(a),G.unbindTree(a),G.unbindEvent(a),a.treeObj.empty(),delete u[a.treeId])},expandCollapseNode:function(a,b,c,d,e){var f=h.getRoot(a),g=a.data.key.children;if(b){if(f.expandTriggerFlag){var m=e;
e=function(){m&&m();b.open?a.treeObj.trigger(q.event.EXPAND,[a.treeId,b]):a.treeObj.trigger(q.event.COLLAPSE,[a.treeId,b])};f.expandTriggerFlag=!1}!b.open&&b.isParent&&(!y(b,q.id.UL,a).get(0)||b[g]&&0<b[g].length&&!y(b[g][0],a).get(0))&&(r.appendParentULDom(a,b),r.createNodeCallback(a));if(b.open==c)n.apply(e,[]);else{c=y(b,q.id.UL,a);var f=y(b,q.id.SWITCH,a),l=y(b,q.id.ICON,a);b.isParent?(b.open=!b.open,b.iconOpen&&b.iconClose&&l.attr("style",r.makeNodeIcoStyle(a,b)),b.open?(r.replaceSwitchClass(b,
f,q.folder.OPEN),r.replaceIcoClass(b,l,q.folder.OPEN),!1==d||""==a.view.expandSpeed?(c.show(),n.apply(e,[])):b[g]&&0<b[g].length?c.slideDown(a.view.expandSpeed,e):(c.show(),n.apply(e,[]))):(r.replaceSwitchClass(b,f,q.folder.CLOSE),r.replaceIcoClass(b,l,q.folder.CLOSE),!1!=d&&""!=a.view.expandSpeed&&b[g]&&0<b[g].length?c.slideUp(a.view.expandSpeed,e):(c.hide(),n.apply(e,[])))):n.apply(e,[])}}else n.apply(e,[])},expandCollapseParentNode:function(a,b,c,d,e){b&&(b.parentTId?(r.expandCollapseNode(a,b,
c,d),b.parentTId&&r.expandCollapseParentNode(a,b.getParentNode(),c,d,e)):r.expandCollapseNode(a,b,c,d,e))},expandCollapseSonNode:function(a,b,c,d,e){var f=h.getRoot(a),g=a.data.key.children,f=b?b[g]:f[g],g=b?!1:d,m=h.getRoot(a).expandTriggerFlag;h.getRoot(a).expandTriggerFlag=!1;if(f)for(var l=0,n=f.length;l<n;l++)f[l]&&r.expandCollapseSonNode(a,f[l],c,g);h.getRoot(a).expandTriggerFlag=m;r.expandCollapseNode(a,b,c,d,e)},makeDOMNodeIcon:function(a,b,c){var d=h.getNodeName(b,c),d=b.view.nameIsHTML?
...
b,c){a.push("<li id='",c.tId,"' class='",q.className.LEVEL,c.level,"' tabindex='0' hidefocus='true' treenode>")},makeDOMNodeNameAfter:function(a){a.push("</a>")},makeDOMNodeNameBefore:function(a,b,c){var d=h.getNodeTitle(b,c),e=r.makeNodeUrl(b,c),f=r.makeNodeFontCss(b,c),g=[],m;for(m in f)g.push(m,":",f[m],";");a.push("<a id='",c.tId,q.id.A,"' class='",q.className.LEVEL,c.level,"' treeNode",q.id.A,' onclick="',c.click||"",'" ',null!=e&&0<e.length?"href='"+e+"'":""," target='",r.makeNodeTarget(c),
"' style='",g.join(""),"'");n.apply(b.view.showTitle,[b.treeId,c],b.view.showTitle)&&d&&a.push("title='",d.replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),"'");a.push(">")},makeNodeFontCss:function(a,b){var c=n.apply(a.view.fontCss,[a.treeId,b],a.view.fontCss);return c&&"function"!=typeof c?c:{}},makeNodeIcoClass:function(a,b){var c=["ico"];b.isAjaxing||(c[0]=(b.iconSkin?b.iconSkin+"_":"")+c[0],b.isParent?c.push(b.open?q.folder.OPEN:q.folder.CLOSE):c.push(q.folder.DOCU));return q.className.BUTTON+
...
a,b,!0,function(){if(!1!==d)try{y(a,e).focus().blur()}catch(b){}});else if(a.open=!b,r.switchNode(this.setting,a),!1!==d)try{y(a,e).focus().blur()}catch(g){}return b},getNodes:function(){return h.getNodes(e)},getNodeByParam:function(a,b,c){return a?h.getNodeByParam(e,c?c[e.data.key.children]:h.getNodes(e),a,b):null},getNodeByTId:function(a){return h.getNodeCache(e,a)},getNodesByParam:function(a,b,c){return a?h.getNodesByParam(e,c?c[e.data.key.children]:h.getNodes(e),a,b):null},getNodesByParamFuzzy:function(a,
b,c){return a?h.getNodesByParamFuzzy(e,c?c[e.data.key.children]:h.getNodes(e),a,b):null},getNodesByFilter:function(a,b,c,d){b=!!b;return a&&"function"==typeof a?h.getNodesByFilter(e,c?c[e.data.key.children]:h.getNodes(e),a,b,d):b?null:[]},getNodeIndex:function(a){if(!a)return null;for(var b=e.data.key.children,c=a.parentTId?a.getParentNode():h.getRoot(e),d=0,f=c[b].length;d<f;d++)if(c[b][d]==a)return d;return-1},getSelectedNodes:function(){for(var a=[],b=h.getRoot(e).curSelectedList,c=0,d=b.length;c<
d;c++)a.push(b[c]);return a},isSelectedNode:function(a){return h.isSelectedNode(e,a)},reAsyncChildNodes:function(a,b,c){if(this.setting.async.enable){var d=!a;d&&(a=h.getRoot(e));if("refresh"==b){b=this.setting.data.key.children;for(var f=0,g=a[b]?a[b].length:0;f<g;f++)h.removeNodeCache(e,a[b][f]);h.removeSelectedNode(e);a[b]=[];d?this.setting.treeObj.empty():y(a,q.id.UL,e).empty()}r.asyncNode(this.setting,d?null:a,!!c)}},refresh:function(){this.setting.treeObj.empty();var a=h.getRoot(e),b=a[e.data.key.children];
h.initRoot(e);a[e.data.key.children]=b;h.initCache(e);r.createNodes(e,0,a[e.data.key.children])},removeChildNodes:function(a){if(!a)return null;var b=a[e.data.key.children];r.removeChildNodes(e,a);return b?b:null},removeNode:function(a,b){a&&(b=!!b,b&&!1==n.apply(e.callback.beforeRemove,[e.treeId,a],!0)||(r.removeNode(e,a),b&&this.setting.treeObj.trigger(q.event.REMOVE,[e.treeId,a])))},selectNode:function(a,b){if(a&&n.uCanDo(e)){b=e.view.selectedMulti&&b;if(a.parentTId)r.expandCollapseParentNode(e,
a.getParentNode(),!0,!1,function(){try{y(a,e).focus().blur()}catch(b){}});else try{y(a,e).focus().blur()}catch(c){}r.selectNode(e,a,b)}},transformTozTreeNodes:function(a){return h.transformTozTreeFormat(e,a)},transformToArray:function(a){return h.transformToArrayFormat(e,a)},updateNode:function(a){a&&y(a,e).get(0)&&n.uCanDo(e)&&(r.setNodeName(e,a),r.setNodeTarget(e,a),r.setNodeUrl(e,a),r.setNodeLineIcos(e,a),r.setNodeFontCss(e,a))}};b.treeTools=d;h.setZTreeTools(e,d);b[c]&&0<b[c].length?r.createNodes(e,
0,b[c]):e.async.enable&&e.async.url&&""!==e.async.url&&r.asyncNode(e);return d}};var ja=a.fn.zTree,y=n.$,q=ja.consts})(jQuery);
...
k.width();E=C.includePadding?k.innerHeight():k.height();I=k.prop("scrollWidth");L=k.prop("scrollHeight");!C.suppressScrollX&&p+C.scrollXMarginOffset<I?(G=!0,n=u(b(p*p/I)),r=b(k.scrollLeft()*(p-n)/(I-p))):(G=!1,r=n=0,k.scrollLeft(0));!C.suppressScrollY&&E+C.scrollYMarginOffset<L?(K=!0,F=u(b(E*E/L)),J=b(k.scrollTop()*(E-F)/(L-E))):(K=!1,J=F=0,k.scrollTop(0));r>=p-n&&(r=p-n);J>=E-F&&(J=E-F);var a={width:p,display:G?"inherit":"none"};a.left=O?k.scrollLeft()+p-I:k.scrollLeft();y?a.bottom=ja-k.scrollTop():
a.top=q+k.scrollTop();l.css(a);a={top:k.scrollTop(),height:E,display:K?"inherit":"none"};C.hiddenScrollY&&(a.display="none");la?(a.right=O?I-k.scrollLeft()-W-H.outerWidth():W-k.scrollLeft(),C.setOffsetRight&&(a.right=C.offsetRight)):a.left=O?k.scrollLeft()+2*p-I-D-H.outerWidth():D+k.scrollLeft();A.css(a);h.css({left:r,width:n-Sa});H.css({top:J,height:F-Q});G&&k.addClass("ps-active-x");K&&k.addClass("ps-active-y")}function v(a,b){var c=k.scrollTop();if(0===a){if(!K)return!1;if(0===c&&0<b||c>=L-E&&
0>b)return!C.wheelPropagation}c=k.scrollLeft();if(0===b){if(!G)return!1;if(0===c&&0>a||c>=I-p&&0<a)return!C.wheelPropagation}return!0}function P(){var a=!1;k.bind(f("mouseenter"),function(b){a=!0});k.bind(f("mouseleave"),function(b){a=!1});var b=!1;jQuery(m).bind(f("keydown"),function(c){if((!c.isDefaultPrevented||!c.isDefaultPrevented())&&a){for(var d=document.activeElement?document.activeElement:m.activeElement;d.shadowRoot;)d=d.shadowRoot.activeElement;if(!jQuery(d).is(":input,[contenteditable]")){var e=
d=0;switch(c.which){case 37:d=-30;break;case 38:e=30;break;case 39:d=30;break;case 40:e=-30;break;case 33:e=90;break;case 32:case 34:e=-90;break;case 35:e=c.ctrlKey?-L:-E;break;case 36:e=c.ctrlKey?k.scrollTop():E;break;default:return}k.scrollTop(k.scrollTop()-e);k.scrollLeft(k.scrollLeft()+d);(b=v(d,e))&&c.preventDefault()}}})}function V(a,b){var c,d;function e(a,b){k.scrollTop(k.scrollTop()-b);k.scrollLeft(k.scrollLeft()-a);t()}function h(a){q=!0}function g(a){q=!1}function m(a){a=a.originalEvent;
return a.targetTouches&&1===a.targetTouches.length||a.pointerType&&"mouse"!==a.pointerType?!0:!1}function l(a){if(m(a)){s=!0;var b=a.originalEvent.targetTouches?a.originalEvent.targetTouches[0]:a.originalEvent;r.pageX=b.pageX;r.pageY=b.pageY;aa=(new Date).getTime();null!==ea&&clearInterval(ea);a.stopPropagation()}}function n(a){if(!q&&s&&m(a)){var b=a.originalEvent.targetTouches?a.originalEvent.targetTouches[0]:a.originalEvent,f={pageX:b.pageX,pageY:b.pageY},b=f.pageX-r.pageX,h=f.pageY-r.pageY;e(b,
h);r=f;var f=(new Date).getTime(),g=f-aa;0<g&&(c=b/g,d=h/g,aa=f);a.stopPropagation();a.preventDefault()}}function p(a){!q&&s&&(s=!1,clearInterval(ea),ea=setInterval(function(){.01>Math.abs(c)&&.01>Math.abs(d)?clearInterval(ea):(e(30*c,30*d),c*=.8,d*=.8)},10))}var r={},aa=0;d=c=void 0;var ea=null,q=!1,s=!1;a&&(jQuery(window).bind(f("touchstart"),h),jQuery(window).bind(f("touchend"),g),k.bind(f("touchstart"),l),k.bind(f("touchmove"),n),k.bind(f("touchend"),p));b&&(window.PointerEvent?(jQuery(window).bind(f("pointerdown"),
h),jQuery(window).bind(f("pointerup"),g),k.bind(f("pointerdown"),l),k.bind(f("pointermove"),n),k.bind(f("pointerup"),p)):window.MSPointerEvent&&(jQuery(window).bind(f("MSPointerDown"),h),jQuery(window).bind(f("MSPointerUp"),g),k.bind(f("MSPointerDown"),l),k.bind(f("MSPointerMove"),n),k.bind(f("MSPointerUp"),p)))}var C=jQuery.extend(!0,{},c),k=jQuery(this);"object"===typeof a?jQuery.extend(!0,C,a):d=a;if("update"===d)return k.data("perfect-scrollbar-update")&&k.data("perfect-scrollbar-update")(),k;
if("destroy"===d)return k.data("perfect-scrollbar-destroy")&&k.data("perfect-scrollbar-destroy")(),k;if(k.data("perfect-scrollbar"))return k.data("perfect-scrollbar");k.addClass("ps-container");var p,E,I,L,O="rtl"===k.css("direction"),f=e(),m=this.ownerDocument||document,l=jQuery("<div class='ps-scrollbar-x-rail'>").appendTo(k),h=jQuery("<div class='ps-scrollbar-x'>").appendTo(l),G,n,r,ja=b(l.css("bottom")),y=ja===ja,q=y?null:b(l.css("top")),Sa=b(l.css("borderLeftWidth"))+b(l.css("borderRightWidth")),
A=jQuery("<div class='ps-scrollbar-y-rail'>").appendTo(k),H=jQuery("<div class='ps-scrollbar-y'>").appendTo(A),K,F,J,W=b(A.css("right")),la=W===W,D=la?null:b(A.css("left")),Q=b(A.css("borderTopWidth"))+b(A.css("borderBottomWidth")),X="ontouchstart"in window||window.DocumentTouch&&document instanceof window.DocumentTouch,x=null!==window.navigator.msMaxTouchPoints;t();(function(){k.bind(f("scroll"),function(a){t()})})();(function(){var a,c,d=!1;h.bind(f("mousedown"),function(b){c=b.pageX;a=h.position().left;
l.addClass("in-scrolling");d=!0;b.stopPropagation();b.preventDefault()});jQuery(m).bind(f("mousemove"),function(e){if(d){var f=a+(e.pageX-c),h=p-n;r=0>f?0:f>h?h:f;f=b(r*(I-p)/(p-n));k.scrollLeft(f);t();e.stopPropagation();e.preventDefault()}});jQuery(m).bind(f("mouseup"),function(a){d&&(d=!1,l.removeClass("in-scrolling"))});a=c=null})();(function(){var a,c,d=!1;H.bind(f("mousedown"),function(b){c=b.pageY;a=H.position().top;d=!0;A.addClass("in-scrolling");b.stopPropagation();b.preventDefault()});jQuery(m).bind(f("mousemove"),
function(e){if(d){var f=a+(e.pageY-c),h=E-F;J=0>f?0:f>h?h:f;f=b(J*(L-E)/(E-F));k.scrollTop(f);t();e.stopPropagation();e.preventDefault()}});jQuery(m).bind(f("mouseup"),function(a){d&&(d=!1,A.removeClass("in-scrolling"))});a=c=null})();(function(){function a(b){b.stopPropagation()}H.bind(f("click"),a);A.bind(f("click"),function(a){var c=b(F/2);a=(a.pageY-A.offset().top-c)/(E-F);0>a?a=0:1<a&&(a=1);k.scrollTop((L-E)*a)});h.bind(f("click"),a);l.bind(f("click"),function(a){var c=b(n/2);a=(a.pageX-l.offset().left-
...
h=!1;jQuery(m).bind(f("selectionchange"),function(b){jQuery.contains(k[0],a())?h=!0:(h=!1,c())});jQuery(window).bind(f("mouseup"),function(a){h&&(h=!1,c())});jQuery(window).bind(f("mousemove"),function(a){if(h){var d=a.pageX;a=a.pageY;var f=k.offset(),g=f.left,m=f.left+k.outerWidth(),n=f.top,f=f.top+k.outerHeight();d<g+3?(e.left=-5,l.addClass("in-scrolling")):d>m-3?(e.left=5,l.addClass("in-scrolling")):e.left=0;a<n+3?(e.top=5>n+3-a?-5:-20,A.addClass("in-scrolling")):a>f-3?(e.top=5>a-f+3?5:20,A.addClass("in-scrolling")):
e.top=0;0===e.top&&0===e.left?c():b()}})})();(function(){function a(c){var d,e=c.originalEvent.deltaX;d=-1*c.originalEvent.deltaY;if("undefined"===typeof e||"undefined"===typeof d)e=-1*c.originalEvent.wheelDeltaX/6,d=c.originalEvent.wheelDeltaY/6;c.originalEvent.deltaMode&&1===c.originalEvent.deltaMode&&(e*=10,d*=10);e!==e&&d!==d&&(e=0,d=c.originalEvent.wheelDelta);d=[e,d];e=d[0];d=d[1];b=!1;C.useBothWheelAxes?K&&!G?(d?k.scrollTop(k.scrollTop()-d*C.wheelSpeed):k.scrollTop(k.scrollTop()+e*C.wheelSpeed),
b=!0):G&&!K&&(e?k.scrollLeft(k.scrollLeft()+e*C.wheelSpeed):k.scrollLeft(k.scrollLeft()-d*C.wheelSpeed),b=!0):(k.scrollTop(k.scrollTop()-d*C.wheelSpeed),k.scrollLeft(k.scrollLeft()+e*C.wheelSpeed));t();if(b=v(e,d))c.stopPropagation(),c.preventDefault()}var b=!1;"undefined"!==typeof window.onwheel?k.bind(f("wheel"),a):"undefined"!==typeof window.onmousewheel&&k.bind(f("mousewheel"),a)})();(X||x)&&V(X,x);C.useKeyboard&&P();k.data("perfect-scrollbar",k);k.data("perfect-scrollbar-update",t);k.data("perfect-scrollbar-destroy",
function(){k.unbind(f());jQuery(window).unbind(f());jQuery(m).unbind(f());k.data("perfect-scrollbar",null);k.data("perfect-scrollbar-update",null);k.data("perfect-scrollbar-destroy",null);h.remove();H.remove();l.remove();A.remove();l=A=h=H=G=K=p=E=I=L=n=r=ja=y=q=F=J=W=la=D=O=f=null});return k})}});
(function(a,b,c){function d(a,c,d,e){for(var h=[],g=0;g<a.length;g++){var n=a[g];if(n){var k=tinycolor(n),p=.5>k.toHsl().l?"sp-thumb-el sp-thumb-dark":"sp-thumb-el sp-thumb-light",p=p+(tinycolor.equals(c,n)?" sp-thumb-active":""),n=k.toString(e.preferredFormat||"rgb"),s=E?"background-color:"+k.toRgbString():"filter:"+k.toFilter();h.push('<span title="'+n+'" data-color="'+k.toRgbString()+'" class="'+p+'"><span class="sp-thumb-inner" style="'+s+';" /></span>')}else h.push(b("<div />").append(b('<span data-color="" style="background-color:transparent;" class="sp-clear-display"></span>').attr("title",
e.noColorSelectedText)).html())}return"<div class='sp-cf "+d+"'>"+h.join("")+"</div>"}function e(a,c){var d=b.extend({},C,a);d.callbacks={move:v(d.move,c),change:v(d.change,c),show:v(d.show,c),hide:v(d.hide,c),beforeShow:v(d.beforeShow,c)};return d}function g(g,f){function m(){x.showPaletteOnly&&(x.showPalette=!0);Ga.text(x.showPaletteOnly?x.togglePaletteMoreText:x.togglePaletteLessText);if(x.palette){wa=x.palette.slice(0);ra=b.isArray(wa[0])?wa:[wa];xa={};for(var a=0;a<ra.length;a++)for(var c=0;c<
ra[a].length;c++){var d=tinycolor(ra[a][c]).toRgbString();xa[d]=!0}}B.toggleClass("sp-flat",Y);B.toggleClass("sp-input-disabled",!x.showInput);B.toggleClass("sp-alpha-enabled",x.showAlpha);B.toggleClass("sp-clear-enabled",ka);B.toggleClass("sp-buttons-disabled",!x.showButtons);B.toggleClass("sp-palette-buttons-disabled",!x.togglePaletteOnly);B.toggleClass("sp-palette-disabled",!x.showPalette);B.toggleClass("sp-palette-only",x.showPaletteOnly);B.toggleClass("sp-initial-disabled",!x.showInitial);B.addClass(x.className).addClass(x.containerClassName);
Q()}function l(){if(S&&a.localStorage){try{var c=a.localStorage[S].split(",#");1<c.length&&(delete a.localStorage[S],b.each(c,function(a,b){h(b)}))}catch(d){}try{ba=a.localStorage[S].split(";")}catch(e){}}}function h(c){if(Ea){c=tinycolor(c).toRgbString();if(!xa[c]&&-1===b.inArray(c,ba))for(ba.push(c);ba.length>Ta;)ba.shift();if(S&&a.localStorage)try{a.localStorage[S]=ba.join(";")}catch(d){}}}function u(){var a=[];if(x.showPalette)for(var b=0;b<ba.length;b++){var c=tinycolor(ba[b]).toRgbString();
xa[c]||a.push(ba[b])}return a.reverse().slice(0,x.maxSelectionSize)}function n(){var a=F(),c=b.map(ra,function(b,c){return d(b,a,"sp-palette-row sp-palette-row-"+c,x)});l();ba&&c.push(d(u(),a,"sp-palette-row sp-palette-row-selection",x));Na.html(c.join(""))}function r(){if(x.showInitial){var a=fa,b=F();Oa.html(d([a,b],b,"sp-palette-row-initial",x))}}function v(){(0>=T||0>=w||0>=U)&&Q();B.addClass("sp-dragging");ga=null;M.trigger("dragstart.spectrum",[F()])}function y(){B.removeClass("sp-dragging");
M.trigger("dragstop.spectrum",[F()])}function q(){var a=ha.val();null!==a&&""!==a||!ka?(a=tinycolor(a),a.isValid()?(K(a),D(!0)):ha.addClass("sp-validation-error")):(K(null),D(!0))}function C(){ca?H():A()}function A(){var c=b.Event("beforeShow.spectrum");if(ca)Q();else if(M.trigger(c,[F()]),!1!==N.beforeShow(F())&&!c.isDefaultPrevented()){for(c=0;c<k.length;c++)k[c]&&k[c].hide();ca=!0;b(Ha).bind("click.spectrum",H);b(a).bind("resize.spectrum",R);ma.addClass("sp-active");B.removeClass("sp-hidden");
Q();W();fa=F();r();N.show(fa);M.trigger("show.spectrum",[fa])}}function H(c){c&&"click"==c.type&&2==c.button||!ca||Y||(ca=!1,b(Ha).unbind("click.spectrum",H),b(a).unbind("resize.spectrum",R),ma.removeClass("sp-active"),B.addClass("sp-hidden"),tinycolor.equals(F(),fa)||(Ua&&"cancel"!==c?D(!0):K(fa,!0)),N.hide(F()),M.trigger("hide.spectrum",[F()]))}function K(a,b){if(tinycolor.equals(a,F()))W();else{var c,d;!a&&ka?ia=!0:(ia=!1,c=tinycolor(a),d=c.toHsv(),pa=d.h%360/360,qa=d.s,oa=d.v,Z=d.a);W();c&&c.isValid()&&
!b&&(sa=Ia||c.getFormat())}}function F(a){a=a||{};return ka&&ia?null:tinycolor.fromRatio({h:pa,s:qa,v:oa,a:Math.round(100*Z)/100},{format:a.format||sa})}function J(){W();N.move(F());M.trigger("move.spectrum",[F()])}function W(){ha.removeClass("sp-validation-error");la();var a=tinycolor.fromRatio({h:pa,s:1,v:1});ya.css("background-color",a.toHexString());a=sa;!(1>Z)||0===Z&&"name"===a||"hex"!==a&&"hex3"!==a&&"hex6"!==a&&"name"!==a||(a="rgb");var b=F({format:a}),c="";na.removeClass("sp-clear-display");
na.css("background-color","transparent");if(!b&&ka)na.addClass("sp-clear-display");else{var c=b.toHexString(),d=b.toRgbString();E||1===b.alpha?na.css("background-color",d):(na.css("background-color","transparent"),na.css("filter",b.toFilter()));if(x.showAlpha){d=b.toRgb();d.a=0;var d=tinycolor(d).toRgbString(),e="linear-gradient(left, "+d+", "+c+")";p?ta.css("filter",tinycolor(d).toFilter({gradientType:1},c)):(ta.css("background","-webkit-"+e),ta.css("background","-moz-"+e),ta.css("background","-ms-"+
e),ta.css("background","linear-gradient(to right, "+d+", "+c+")"))}c=b.toString(a)}x.showInput&&ha.val(c);x.showPalette&&n();r()}function la(){var a=qa,b=oa;ka&&ia?(za.hide(),Aa.hide(),Ba.hide()):(za.show(),Aa.show(),Ba.show(),a*=w,b=T-b*T,a=Math.max(-da,Math.min(w-da,a-da)),b=Math.max(-da,Math.min(T-da,b-da)),Ba.css({top:b+"px",left:a+"px"}),za.css({left:Z*Fa-aa/2+"px"}),Aa.css({top:pa*U-ea+"px"}))}function D(a){var b=F(),c="",d=!tinycolor.equals(b,fa);b&&(c=b.toString(sa),h(b));Ca&&M.val(c);fa=
b;a&&d&&(N.change(b),M.trigger("change",[b]))}function Q(){w=ya.width();T=ya.height();da=Ba.height();Ja.width();U=Ja.height();ea=Aa.height();Fa=Pa.width();aa=za.width();Y||(B.css("position","absolute"),B.offset(s(B,ua)));la();x.showPalette&&n();M.trigger("reflow.spectrum")}function X(){H();Ka=!0;M.attr("disabled",!0);ua.addClass("sp-disabled")}var x=e(f,g),Y=x.flat,Ea=x.showSelectionPalette,S=x.localStorageKey,z=x.theme,N=x.callbacks,R=V(Q,10),ca=!1,w=0,T=0,da=0,U=0,Fa=0,aa=0,ea=0,pa=0,qa=0,oa=0,
Z=1,wa=[],ra=[],xa={},ba=x.selectionPalette.slice(0),Ta=x.maxSelectionSize,ga=null,Ha=g.ownerDocument,M=b(g),Ka=!1,B=b(L,Ha).addClass(z),Va=B.find(".sp-picker-container"),ya=B.find(".sp-color"),Ba=B.find(".sp-dragger"),Ja=B.find(".sp-hue"),Aa=B.find(".sp-slider"),ta=B.find(".sp-alpha-inner"),Pa=B.find(".sp-alpha"),za=B.find(".sp-alpha-handle"),ha=B.find(".sp-input"),Na=B.find(".sp-palette"),Oa=B.find(".sp-initial"),Qa=B.find(".sp-cancel"),La=B.find(".sp-clear"),Ra=B.find(".sp-choose"),Ga=B.find(".sp-palette-toggle"),
Ca=M.is("input"),Wa=Ca&&I&&"color"===M.attr("type"),Ma=Ca&&!Y,ma=Ma?b("<div class='sp-replacer'><div class='sp-preview'><div class='sp-preview-inner'></div></div><div class='sp-dd'>&#9660;</div></div>").addClass(z).addClass(x.className).addClass(x.replacerClassName):b([]),ua=Ma?ma:M,na=ma.find(".sp-preview-inner"),va=x.color||Ca&&M.val(),fa=!1,Ia=x.preferredFormat,sa=Ia,Ua=!x.showButtons||x.clickoutFiresChange,ia=!va,ka=x.allowEmpty&&!Wa;(function(){function a(c){c.data&&c.data.ignore?(K(b(c.target).closest(".sp-thumb-el").data("color")),
J()):(K(b(c.target).closest(".sp-thumb-el").data("color")),J(),D(!0),x.hideAfterPaletteSelect&&H());return!1}p&&B.find("*:not(input)").attr("unselectable","on");m();Ma&&M.after(ma).hide();ka||La.hide();if(Y)M.after(B).hide();else{var c="parent"===x.appendTo?M.parent():b(x.appendTo);1!==c.length&&(c=b("body"));c.append(B)}l();ua.bind("click.spectrum touchstart.spectrum",function(a){Ka||C();a.stopPropagation();b(a.target).is("input")||a.preventDefault()});(M.is(":disabled")||!0===x.disabled)&&X();B.click(t);
ha.change(q);ha.bind("paste",function(){setTimeout(q,1)});ha.keydown(function(a){13==a.keyCode&&q()});Qa.text(x.cancelText);Qa.bind("click.spectrum",function(a){a.stopPropagation();a.preventDefault();H("cancel")});La.attr("title",x.clearText);La.bind("click.spectrum",function(a){a.stopPropagation();a.preventDefault();ia=!0;J();Y&&D(!0)});Ra.text(x.chooseText);Ra.bind("click.spectrum",function(a){a.stopPropagation();a.preventDefault();ha.hasClass("sp-validation-error")||(D(!0),H())});Ga.text(x.showPaletteOnly?
x.togglePaletteMoreText:x.togglePaletteLessText);Ga.bind("click.spectrum",function(a){a.stopPropagation();a.preventDefault();x.showPaletteOnly=!x.showPaletteOnly;x.showPaletteOnly||Y||B.css("left","-="+(Va.outerWidth(!0)+5));m()});P(Pa,function(a,b,c){Z=a/Fa;ia=!1;c.shiftKey&&(Z=Math.round(10*Z)/10);J()},v,y);P(Ja,function(a,b){pa=parseFloat(b/U);ia=!1;x.showAlpha||(Z=1);J()},v,y);P(ya,function(a,b,c){c.shiftKey?ga||(c=T-oa*T,ga=Math.abs(a-qa*w)>Math.abs(b-c)?"x":"y"):ga=null;c=!ga||"y"===ga;ga&&
"x"!==ga||(qa=parseFloat(a/w));c&&(oa=parseFloat((T-b)/T));ia=!1;x.showAlpha||(Z=1);J()},v,y);va?(K(va),W(),sa=Ia||tinycolor(va).format,h(va)):W();Y&&A();c=p?"mousedown.spectrum":"click.spectrum touchstart.spectrum";Na.delegate(".sp-thumb-el",c,a);Oa.delegate(".sp-thumb-el:nth-child(1)",c,{ignore:!0},a)})();var Da={show:A,hide:H,toggle:C,reflow:Q,option:function(a,d){if(a===c)return b.extend({},x);if(d===c)return x[a];x[a]=d;m()},enable:function(){Ka=!1;M.attr("disabled",!1);ua.removeClass("sp-disabled")},
disable:X,set:function(a){K(a);D()},get:F,destroy:function(){M.show();ua.unbind("click.spectrum touchstart.spectrum");B.remove();ma.remove();k[Da.id]=null},container:B};Da.id=k.push(Da)-1;return Da}function s(a,c){var d=a.outerWidth(),e=a.outerHeight(),h=c.outerHeight(),g=a[0].ownerDocument,n=g.documentElement,k=n.clientWidth+b(g).scrollLeft(),g=n.clientHeight+b(g).scrollTop(),n=c.offset();n.top+=h;n.left-=Math.min(n.left,n.left+d>k&&k>d?Math.abs(n.left+d-k):0);n.top-=Math.min(n.top,n.top+e>g&&g>
e?Math.abs(e+h-0):0);return n}function u(){}function t(a){a.stopPropagation()}function v(a,b){var c=Array.prototype.slice,d=c.call(arguments,2);return function(){return a.apply(b,d.concat(c.call(arguments)))}}function P(c,d,e,g){function h(a){a.stopPropagation&&a.stopPropagation();a.preventDefault&&a.preventDefault();a.returnValue=!1}function k(a){if(s){if(p&&9>r.documentMode&&!a.button)return n();var b=a.originalEvent.touches,e=b?b[0].pageY:a.pageY,b=Math.max(0,Math.min((b?b[0].pageX:a.pageX)-u.left,
v)),e=Math.max(0,Math.min(e-u.top,q));t&&h(a);d.apply(c,[b,e,a])}}function n(){s&&(b(r).unbind(E),b(r.body).removeClass("sp-dragging"),g.apply(c,arguments));s=!1}d=d||function(){};e=e||function(){};g=g||function(){};var r=document,s=!1,u={},q=0,v=0,t="ontouchstart"in a,E={};E.selectstart=h;E.dragstart=h;E["touchmove mousemove"]=k;E["touchend mouseup"]=n;b(c).bind("touchstart mousedown",function(a){(a.which?3==a.which:2==a.button)||s||!1===e.apply(c,arguments)||(s=!0,q=b(c).height(),v=b(c).width(),
u=b(c).offset(),b(r).bind(E),b(r.body).addClass("sp-dragging"),t||k(a),h(a))})}function V(a,b,c){var d;return function(){var e=this,g=arguments,k=function(){d=null;a.apply(e,g)};c&&clearTimeout(d);if(c||!d)d=setTimeout(k,b)}}var C={beforeShow:u,move:u,change:u,show:u,hide:u,color:!1,flat:!1,showInput:!1,allowEmpty:!1,showButtons:!0,clickoutFiresChange:!1,showInitial:!1,showPalette:!1,showPaletteOnly:!1,hideAfterPaletteSelect:!1,togglePaletteOnly:!1,showSelectionPalette:!0,localStorageKey:!1,appendTo:"body",
...
1);this.each(function(){var c=k[b(this).data("spectrum.id")];if(c){var f=c[a];if(!f)throw Error("Spectrum: no such method: '"+a+"'");"get"==a?d=c.get():"container"==a?d=c.container:"option"==a?d=c.option.apply(c,e):"destroy"==a?(c.destroy(),b(this).removeData("spectrum.id")):f.apply(c,e)}});return d}return this.spectrum("destroy").each(function(){var c=b.extend({},a,b(this).data()),c=g(this,c);b(this).data("spectrum.id",c.id)})};b.fn.spectrum.load=!0;b.fn.spectrum.loadOpts={};b.fn.spectrum.draggable=
P;b.fn.spectrum.defaults=C;b.spectrum={};b.spectrum.localization={};b.spectrum.palettes={};b.fn.spectrum.processNativeColorInputs=function(){I||b("input[type=color]").spectrum({preferredFormat:"hex6"})};(function(){function b(a,c,d){a=D(a,255);c=D(c,255);d=D(d,255);var e=R(a,c,d),f=N(a,c,d),h,g=(e+f)/2;if(e==f)h=f=0;else{var k=e-f,f=.5<g?k/(2-e-f):k/(e+f);switch(e){case a:h=(c-d)/k+(c<d?6:0);break;case c:h=(d-a)/k+2;break;case d:h=(a-c)/k+4}h/=6}return{h:h,s:f,l:g}}function c(a,b,d){function e(a,
b,c){0>c&&(c+=1);1<c&&(c-=1);return c<1/6?a+6*(b-a)*c:.5>c?b:c<2/3?a+(b-a)*(2/3-c)*6:a}a=D(a,360);b=D(b,100);d=D(d,100);if(0===b)d=b=a=d;else{var f=.5>d?d*(1+b):d+b-d*b,h=2*d-f;d=e(h,f,a+1/3);b=e(h,f,a);a=e(h,f,a-1/3)}return{r:255*d,g:255*b,b:255*a}}function d(a,b,c){a=D(a,255);b=D(b,255);c=D(c,255);var e=R(a,b,c),f=N(a,b,c),h,g=e-f;if(e==f)h=0;else{switch(e){case a:h=(b-c)/g+(b<c?6:0);break;case b:h=(c-a)/g+2;break;case c:h=(a-b)/g+4}h/=6}return{h:h,s:0===e?0:g/e,v:e}}function e(a,b,c,d){a=[Q(z(a).toString(16)),
Q(z(b).toString(16)),Q(z(c).toString(16))];return d&&a[0].charAt(0)==a[0].charAt(1)&&a[1].charAt(0)==a[1].charAt(1)&&a[2].charAt(0)==a[2].charAt(1)?a[0].charAt(0)+a[1].charAt(0)+a[2].charAt(0):a.join("")}function h(a,b,c,d){return[Q(Math.round(255*parseFloat(d)).toString(16)),Q(z(a).toString(16)),Q(z(b).toString(16)),Q(z(c).toString(16))].join("")}function g(a,b){b=0===b?0:b||10;var c=w(a).toHsl();c.s-=b/100;c.s=N(1,R(0,c.s));return w(c)}function k(a,b){b=0===b?0:b||10;var c=w(a).toHsl();c.s+=b/100;
c.s=N(1,R(0,c.s));return w(c)}function p(a){return w(a).desaturate(100)}function s(a,b){b=0===b?0:b||10;var c=w(a).toHsl();c.l+=b/100;c.l=N(1,R(0,c.l));return w(c)}function u(a,b){b=0===b?0:b||10;var c=w(a).toRgb();c.r=R(0,N(255,c.r-z(255*-(b/100))));c.g=R(0,N(255,c.g-z(255*-(b/100))));c.b=R(0,N(255,c.b-z(255*-(b/100))));return w(c)}function q(a,b){b=0===b?0:b||10;var c=w(a).toHsl();c.l-=b/100;c.l=N(1,R(0,c.l));return w(c)}function v(a,b){var c=w(a).toHsl(),d=(z(c.h)+b)%360;c.h=0>d?360+d:d;return w(c)}
function t(a){a=w(a).toHsl();a.h=(a.h+180)%360;return w(a)}function E(a){var b=w(a).toHsl(),c=b.h;return[w(a),w({h:(c+120)%360,s:b.s,l:b.l}),w({h:(c+240)%360,s:b.s,l:b.l})]}function P(a){var b=w(a).toHsl(),c=b.h;return[w(a),w({h:(c+90)%360,s:b.s,l:b.l}),w({h:(c+180)%360,s:b.s,l:b.l}),w({h:(c+270)%360,s:b.s,l:b.l})]}function V(a){var b=w(a).toHsl(),c=b.h;return[w(a),w({h:(c+72)%360,s:b.s,l:b.l}),w({h:(c+216)%360,s:b.s,l:b.l})]}function C(a,b,c){b=b||6;c=c||30;var d=w(a).toHsl();c=360/c;a=[w(a)];for(d.h=
(d.h-(c*b>>1)+720)%360;--b;)d.h=(d.h+c)%360,a.push(w(d));return a}function I(a,b){b=b||6;for(var c=w(a).toHsv(),d=c.h,e=c.s,c=c.v,f=[],h=1/b;b--;)f.push(w({h:d,s:e,v:c})),c=(c+h)%1;return f}function L(a){a=parseFloat(a);if(isNaN(a)||0>a||1<a)a=1;return a}function D(a,b){var c=a;"string"==typeof c&&-1!=c.indexOf(".")&&1===parseFloat(c)&&(a="100%");c="string"===typeof a&&-1!=a.indexOf("%");a=N(b,R(0,parseFloat(a)));c&&(a=parseInt(a*b,10)/100);return 1E-6>S.abs(a-b)?1:a%b/parseFloat(b)}function Q(a){return 1==
a.length?"0"+a:""+a}function X(a){1>=a&&(a=100*a+"%");return a}var x=/^[\s,#]+/,Y=/\s+$/,Ea=0,S=Math,z=S.round,N=S.min,R=S.max,ca=S.random,w=function aa(a,b){var d,e,h,g,k;a=a?a:"";b=b||{};if(a instanceof aa)return a;if(!(this instanceof aa))return new aa(a,b);e=a;k={r:0,g:0,b:0};var m=1;h=d=!1;if("string"==typeof e)a:{e=e.replace(x,"").replace(Y,"").toLowerCase();var l=!1;if(T[e])e=T[e],l=!0;else if("transparent"==e){e={r:0,g:0,b:0,a:0,format:"name"};break a}e=(g=U.rgb.exec(e))?{r:g[1],g:g[2],b:g[3]}:
(g=U.rgba.exec(e))?{r:g[1],g:g[2],b:g[3],a:g[4]}:(g=U.hsl.exec(e))?{h:g[1],s:g[2],l:g[3]}:(g=U.hsla.exec(e))?{h:g[1],s:g[2],l:g[3],a:g[4]}:(g=U.hsv.exec(e))?{h:g[1],s:g[2],v:g[3]}:(g=U.hex8.exec(e))?{a:parseInt(g[1],16)/255,r:parseInt(g[2],16),g:parseInt(g[3],16),b:parseInt(g[4],16),format:l?"name":"hex8"}:(g=U.hex6.exec(e))?{r:parseInt(g[1],16),g:parseInt(g[2],16),b:parseInt(g[3],16),format:l?"name":"hex"}:(g=U.hex3.exec(e))?{r:parseInt(g[1]+""+g[1],16),g:parseInt(g[2]+""+g[2],16),b:parseInt(g[3]+
""+g[3],16),format:l?"name":"hex"}:!1}if("object"==typeof e){if(e.hasOwnProperty("r")&&e.hasOwnProperty("g")&&e.hasOwnProperty("b"))k=e.g,d=e.b,k={r:255*D(e.r,255),g:255*D(k,255),b:255*D(d,255)},d=!0,h="%"===String(e.r).substr(-1)?"prgb":"rgb";else if(e.hasOwnProperty("h")&&e.hasOwnProperty("s")&&e.hasOwnProperty("v")){e.s=X(e.s);e.v=X(e.v);h=e.h;l=e.s;k=e.v;h=6*D(h,360);l=D(l,100);k=D(k,100);d=S.floor(h);var n=h-d;h=k*(1-l);g=k*(1-n*l);l=k*(1-(1-n)*l);d%=6;k={r:255*[k,g,h,h,l,k][d],g:255*[l,k,k,
g,h,h][d],b:255*[h,h,l,k,k,g][d]};d=!0;h="hsv"}else e.hasOwnProperty("h")&&e.hasOwnProperty("s")&&e.hasOwnProperty("l")&&(e.s=X(e.s),e.l=X(e.l),k=c(e.h,e.s,e.l),d=!0,h="hsl");e.hasOwnProperty("a")&&(m=e.a)}m=L(m);e=e.format||h;h=N(255,R(k.r,0));g=N(255,R(k.g,0));k=N(255,R(k.b,0));this._r=h;this._g=g;this._b=k;this._a=m;this._roundA=z(100*this._a)/100;this._format=b.format||e;this._gradientType=b.gradientType;1>this._r&&(this._r=z(this._r));1>this._g&&(this._g=z(this._g));1>this._b&&(this._b=z(this._b));
this._ok=d;this._tc_id=Ea++};w.prototype={isDark:function(){return 128>this.getBrightness()},isLight:function(){return!this.isDark()},isValid:function(){return this._ok},getFormat:function(){return this._format},getAlpha:function(){return this._a},getBrightness:function(){var a=this.toRgb();return(299*a.r+587*a.g+114*a.b)/1E3},setAlpha:function(a){this._a=L(a);this._roundA=z(100*this._a)/100;return this},toHsv:function(){var a=d(this._r,this._g,this._b);return{h:360*a.h,s:a.s,v:a.v,a:this._a}},toHsvString:function(){var a=
...
this._a?"rgb("+z(100*D(this._r,255))+"%, "+z(100*D(this._g,255))+"%, "+z(100*D(this._b,255))+"%)":"rgba("+z(100*D(this._r,255))+"%, "+z(100*D(this._g,255))+"%, "+z(100*D(this._b,255))+"%, "+this._roundA+")"},toName:function(){return 0===this._a?"transparent":1>this._a?!1:da[e(this._r,this._g,this._b,!0)]||!1},toFilter:function(a){var b="#"+h(this._r,this._g,this._b,this._a),c=b,d=this._gradientType?"GradientType = 1, ":"";a&&(c=w(a).toHex8String());return"progid:DXImageTransform.Microsoft.gradient("+
...
b[f]),h=125<g.brightness&&500<g.color,g=g.brightness/125*3+g.color/500;if(h&&!e||h&&e&&g>d||!h&&!e&&g>d)e=h,d=g,c=w(b[f])}return c};var T=w.names={aliceblue:"f0f8ff",antiquewhite:"faebd7",aqua:"0ff",aquamarine:"7fffd4",azure:"f0ffff",beige:"f5f5dc",bisque:"ffe4c4",black:"000",blanchedalmond:"ffebcd",blue:"00f",blueviolet:"8a2be2",brown:"a52a2a",burlywood:"deb887",burntsienna:"ea7e5d",cadetblue:"5f9ea0",chartreuse:"7fff00",chocolate:"d2691e",coral:"ff7f50",cornflowerblue:"6495ed",cornsilk:"fff8dc",
...
seagreen:"2e8b57",seashell:"fff5ee",sienna:"a0522d",silver:"c0c0c0",skyblue:"87ceeb",slateblue:"6a5acd",slategray:"708090",slategrey:"708090",snow:"fffafa",springgreen:"00ff7f",steelblue:"4682b4",tan:"d2b48c",teal:"008080",thistle:"d8bfd8",tomato:"ff6347",turquoise:"40e0d0",violet:"ee82ee",wheat:"f5deb3",white:"fff",whitesmoke:"f5f5f5",yellow:"ff0",yellowgreen:"9acd32"},da=w.hexNames=function(a){var b={},c;for(c in a)a.hasOwnProperty(c)&&(b[a[c]]=c);return b}(T),U={rgb:/rgb[\s|\(]+((?:[-\+]?\d*\.\d+%?)|(?:[-\+]?\d+%?))[,|\s]+((?:[-\+]?\d*\.\d+%?)|(?:[-\+]?\d+%?))[,|\s]+((?:[-\+]?\d*\.\d+%?)|(?:[-\+]?\d+%?))\s*\)?/,
...
0<e?e=0:c=a.clientX;0>=t.offset().top&&0>u||jQuery(window).height()-t.height()<=t.offset().top&&0<u?u=0:d=a.clientY;t.css("left",Math.max(t.offset().left+e,0));t.css("top",Math.max(t.offset().top+u,0))}else jQuery(b.transparentPanelSelector).hide()})};document.onmouseup=function(){var a=jQuery(b.getMoveComponentSelector()),c=a.offset().left,d=a.offset().top;0>a.offset().left?c=0:jQuery(window).width()-a.width()<a.offset().left&&(c=jQuery(window).width()-a.width());0>a.offset().top?d=0:jQuery(window).height()-
a.height()<a.offset().top&&(d=jQuery(window).height()-a.height());a.css("left",c);a.css("top",d);b.dragging=!1;jQuery(b.transparentPanelSelector).hide()}};SNSComponent.prototype.shadeShow=function(){jQuery(this.transparentPanelSelector).show()};SNSComponent.prototype.shadeHide=function(){jQuery(this.transparentPanelSelector).hide()};var SNSTab=function(){this.index;this.name=Math.uuid();this.type="tab";this.closeable=!1};SNSTab.prototype=new SNSComponent;SNSTab.prototype._init=function(){};
SNSTab.prototype.visible=function(){return this.isContentVisible()};SNSTab.prototype.isHeadVisible=function(){if(this.getHeadDom().is(":visible"))return!0};SNSTab.prototype.isContentVisible=function(){if(this.getContentDom().is(":visible"))return!0};SNSTab.prototype.getHeadDom=function(){this._headDom&&0!=this._headDom.length||(this._headDom=jQuery(this.headSelector));return this._headDom};
SNSTab.prototype.getContentDom=function(){this._contentDom&&0!=this._contentDom.length||(this._contentDom=jQuery(this.contentSelector));return this._contentDom};SNSTab.prototype.getHeadHtml=function(){return TemplateUtil.genHtml(this.getHeadTemplate(),this)};SNSTab.prototype.getContentHtml=function(){return TemplateUtil.genHtml(this.getContentTemplate(),this)};SNSTab.prototype.getHeadTemplate=function(){return this.headTemplate?this.headTemplate:SNSTab.headTemplate};
SNSTab.prototype.getContentTemplate=function(){return this.contentTemplate?this.contentTemplate:SNSTab.contentTemplate};SNSTab.prototype.beforeSelect=function(){};SNSTab.prototype.select=function(){var a=this.getHeadDom();if(!a.hasClass("cur")){var b=this.getContentDom();this.beforeSelect();a.addClass("cur");b.addClass("cur");this.afterSelect()}};SNSTab.prototype.afterSelect=function(){};SNSTab.prototype.beforeUnselect=function(){};
SNSTab.prototype.unselect=function(){var a=this.getHeadDom();if(a.hasClass("cur")){var b=this.getContentDom();this.beforeUnselect();a.removeClass("cur");b.removeClass("cur")}};var SNSTabList=function(){};SNSTabList.prototype=new SNSBaseList;SNSTabList.prototype.add=function(a){a&&a instanceof SNSTab&&SNSBaseList.prototype.add.call(this,a.name,a)};
...
var TemplateUtil={genHtml:function(a,b){function c(a,b){a=(""+a).trim().replace(/##{{([\w()_.]+)}}/,"$1");for(var c,d=0;d<b.length;d++)try{if((c=eval("arguments[1]["+d+"]."+a))&&("string"==typeof c&&c.notEmpty()||"number"==typeof c))return c}catch(e){}if(0==a.indexOf("LANG_"))return SNS_LANG_TEMPLATE[a.replace("LANG_","")];if(0==a.indexOf("lang_"))return SNS_LANG_TEMPLATE[a.replace("lang_","")];try{var g=eval(a);if(g&&"string"==typeof g)return g}catch(C){}return""}if(null==a||a.isEmpty())YYIMChat.log("template can not be null",
0);else{b instanceof Array||(b=[b]);var d=a,e=a.match(/##\{\{([\w()_.]+)\}\}/g),g;for(g in e)e[g].length!=a.length&&(d=d.replace(e[g],c(e[g],b)));return d}}},SNSChatRoomTab=function(){this.name="chatroom";this.selector="#snsim_wide_tab_container";this.headSelector="#snsim_tab_head_chatroom";this.contentSelector="#snsim_tab_content_chatroom";this.chatroomContainerSelector="#snsim_chatroom_list_container";this.roomItemIdPrefix="snsim_window_wide_tab_chatroom_";this.createChatroomPanel=new SNSCreateChatroomPanel;
...
SNSChatWindow.prototype._init=function(){YYIMChat.log("SNSChatWindow.prototype._init",3);this.transPosition(!0);this.enableMove();jQuery(".snsim_chat_friend_box").perfectScrollbar({suppressScrollX:!0,setOffsetRight:!0,offsetRight:0});this._bindWindowButtonEvent();SNSApplication.getInstance().getGlobalEventManager().registerEventHandler(SNS_EVENT_SUBJECT.ON_MESSAGE_IN,!0,this.onMessageIn,this)};SNSChatWindow.prototype.onMessageIn=function(a,b){var c=b.message;c&&c instanceof SNSMessage&&this.addMessage(c)};
SNSChatWindow.prototype.addMessage=function(a){var b=a.getRosterOrChatRoom();if(b=this.getTab(b))b.addMessage(a),this.getCurrentTab()!==b?b.addMsgRemind():a.body.receiptsbody&&a.body.receiptsbody.type==PACKETTYPE.LONGCONNECT&&!0===a.body.receiptsbody.receipts&&YYIMChat.sendReadedReceiptsPacket(a.body.receiptsbody.arg);if("groupchat"==a.type)try{jQuery("#snsim_tab_content_share_"+a.chatroom.getID()).find(".snsim_share_file_refresh_btn").click()}catch(c){YYIMChat.log("groupshare_refresh",1,a,c)}};
SNSChatWindow.prototype._bindWindowButtonEvent=function(){jQuery(this.miniBtnSelector).on("click",function(){SNSIMWindow.getInstance()._toggleMiniChatWindow()});jQuery(this.closeBtnSelector).on("click",jQuery.proxy(function(){this.hide()},this));jQuery(this.showChatroomMembersSelector).on("click",jQuery.proxy(function(){this.getActiveRoster()instanceof SNSChatRoom&&SNSIMWindow.getInstance().getChatroomMembersPanel().show()},this))};
...
SNSChatWindow.prototype.openChatWith=function(a){var b;b=a.id?a.id:a;var c=this.getTab(b);if(!c){if("string"==typeof a&&(a=SNSApplication.getInstance().getUser().getRosterOrChatRoom(a),!a))throw"invalid roster";c=this._addNewChat(a)}if(this.getCurrentTab()!=c){var d=this.getCurrentTab();this.changeTabTo(c);this.updateTitle(d,c)}this._renderUnreadMessage(a);SNSIMWindow.getInstance().getUnreadMessagePanel().removeUnreadMessage(b);this.visible()||this.show();setTimeout(function(){jQuery("#snsim_sendbox_content").focus()},
0)};SNSChatWindow.prototype.onTabChange=function(a,b){b.removeMsgRemind();this.updateTitle(a,b);SNSApplication.getInstance().getGlobalEventManager().trigger(SNS_EVENT_SUBJECT.ON_CURRENT_CHAT_CHANGE,[{oldValue:a,newValue:b}])};
...
SNSChatWindow.prototype._bindTabHeadEvent=function(a){a.getCloseBtnDom().bind("click",{tab:a},jQuery.proxy(function(b){b.data.tab.getHeadDom().remove();b.data.tab.getContentDom().remove();b=this.removeTab(a);0==this.tabs.size()&&this.hide();b&&this.changeTabTo(b);SNSApplication.getInstance().getGlobalEventManager().trigger(SNS_EVENT_SUBJECT.TAB_CLOSED,[a])},this));a.getHeadDom().bind("mouseenter",{tab:a},function(a){a.data.tab.getHeadDom().hasClass("snsim_active")||a.data.tab.getCloseBtnDom().css("display",
"block")});a.getHeadDom().bind("mouseleave",{tab:a},function(a){a.data.tab.getCloseBtnDom().css("display","none")})};
...
SNSRecentTab.prototype.updateRecent=function(){var a=SNSApplication.getInstance().getUser().recentList.getFirstItem(),b=this.getContentDom().find("#snsim_tab_recent_roster_"+a.id),c=jQuery(this.containerSelector);0<b.length?c.prepend(b):(b=TemplateUtil.genHtml(this.rosterTemplate,[SNSApplication.getInstance().getUser().getRosterOrChatRoom(a.id),a.message]),c.prepend(b),this._bindRosterDomEvent(a));this.getContentDom().find("li[id^='snsim_tab_recent_roster_']").length>SNSConfig.RECENT.MAX_SIZE&&this.getContentDom().find("li[id^='snsim_tab_recent_roster_']:last").remove()};
...
SNSSendBox.prototype.send=function(){if(this.getContent())if(this.getContentLength()>SNSConfig.MESSAGE.MAX_CHARACHER)alert("\u6d88\u606f\u8d85\u51fa\u957f\u5ea6\u9650\u5236");else{var a=SNS_CHAT_TYPE.CHAT,b=SNSIMWindow.getInstance().getChatWindow().getActiveRoster(),c=b.id;b instanceof SNSPublicAccountRoster?a=SNS_CHAT_TYPE.PUB_ACCOUNT:b instanceof SNSChatRoom&&(a=SNS_CHAT_TYPE.GROUP_CHAT);var d={to:c,msg:this.getContent(),type:a,success:jQuery.proxy(function(a){var b=new SNSOutMessage(a);b.setText(a.body.content);
d.style&&(b.body.style=new SNSMessageStyle(d.style.font,d.style.size,d.style.color,d.style.biu));SNSApplication.getInstance().getUser().recentList.addNew(b);SNSApplication.getInstance().getMessageInBox().filter.doFilter(b);SNSIMWindow.getInstance().getChatWindow().getTab(b.to).addMessage(b);this.clearContent()},this)};b.resource&&(d.resource=b.resource);SNSMessageStyle.used&&(d.style=SNSMessageStyle.getInstance());YYIMChat.sendTextMessage(d)}};
...
SNSSendBox.prototype.insertHtmlContent=function(a){var b=jQuery(this.contentDomSelector),c;if(window.getSelection){if(b=window.getSelection(),b.getRangeAt&&b.rangeCount){c=b.getRangeAt(0);c.deleteContents();var d=document.createElement("div");d.innerHTML=a;a=document.createDocumentFragment();for(var e,g;e=d.firstChild;)g=a.appendChild(e);(d=c.commonAncestorContainer.parentElement)&&"true"==d.contentEditable?c.insertNode(a):this.getContentDom().append(a);g&&(c=c.cloneRange(),c.setStartAfter(g),c.collapse(!0),
...
SNSRosterChatTab.prototype.addMessage=function(a){var b;b=a instanceof SNSOutMessage?new SNSSentMessageBox(a):new SNSReceivedMessageBox(a);var c=b.getHtml();this.getContentDom().find(".snsim_message_box_container").append(c);45E3>a.body.dateline-this.lastMessageDateline&&b.getDom().find(".dia_info").css("display","none");b.show();this.scrollToBottom();this.lastMessageDateline=a.body.dateline};SNSRosterChatTab.prototype.getTarget=function(){return this.roster};
var SNSChatRoomChatTab=function(a){this.name=a.id;this.chatroom=a;this.tabContainer=new SNSInnerChatTabContainer(a);this.closeable=!0;this.headSelector="#"+SNSChatRoomChatTab.headIdPrefix+a.getID();this.contentSelector="#"+SNSChatRoomChatTab.contentIdPrefix+a.getID()};SNSChatRoomChatTab.headIdPrefix="snsim_chat_window_inner_tab_head_";SNSChatRoomChatTab.contentIdPrefix="snsim_chat_window_inner_tab_content_";SNSChatRoomChatTab.headTemplate='<li id="'+SNSChatRoomChatTab.headIdPrefix+'##{{chatroom.getID()}}" title="##{{chatroom.name}}" class="snsim_tab_head"><div class="list_head_item"><div class="list_head_state" style="margin-left: 12px;margin-right: 2px;"><span class="snsim_roster_presence W_chat_stat snsim_chatroom"></span></div><div class="snsim_username">##{{chatroom.name}}<span class="wbim_icon_vf"></span></div><a hidefloat="true" class="snsim_chat_window_tab_close_btn"></a></div></li>';
...
SNSUnreadMessagePanel.prototype.renderUnreadMessage=function(a){var b=a.getRosterOrChatRoom(),c=b.getID(),d=this.getContainerDom().find("li[rosterId='"+c+"']"),e;e=b instanceof SNSChatRoom?SNSIMWindow.getInstance().getWideWindow().getTab("chatroom"):SNSIMWindow.getInstance().getWideWindow().getTab("roster");d&&0<d.length?(c=parseInt(d.find(".remind_message_num").text()),d.find(".remind_message_num").text(c+1),d.find(".remind_message_content").text(a.body.content),e.renderUnreadMsgNum(b,c+1)):(a=TemplateUtil.genHtml(this.messageTemplate,
a),this.getContainerDom().append(a),this._bindClickEvent(c),this.updateUnreadMessageIndicator(),e.renderUnreadMsgNum(b,1))};SNSUnreadMessagePanel.prototype._bindClickEvent=function(a){this.getContainerDom().find("li[rosterId='"+a+"']").bind("click",{self:this,rId:a},function(a){var c=a.data.self,d=a.data.rId,e=SNSApplication.getInstance().getUser();e.getRosterOrChatRoom(d)||e.rosterList.add(new SNSRoster(d));SNSIMWindow.getInstance().getChatWindow().openChatWith(a.data.rId);c.removeUnreadMessage(d)})};
...
SNSInviteFromOrgTab.prototype.loadNodeByParent=function(a){if(!a.loaded&&!a.loading){this.addLoadingNode(a);a.loading=!0;var b=new JSJaCIQ;b.setType(SNS_TYPE.GET);b.setTo(a.id+"@org."+SNSApplication.getInstance().getDomain());var c=b.buildNode("query",{xmlns:NS_ORGANIZATION});b.appendNode(c);YYIMChat.send(b,jQuery.proxy(function(b,c){if(b.isError())YYIMChat.log("SNSInviteFromOrgTab.prototype.loadNodeByParent ",0,c);else{for(var g=[],s=jQuery(b.doc.xml).find("item[id]"),u=0;u<s.length;u++){var t=jQuery(s[u]),
v={};v.id=t.attr("id");v.name=t.attr("name");var P=t.attr("isUser");"false"==t.attr("isLeaf")?(v.isParent=!0,v.iconSkin="department"):v.isParent=!1;"true"==P&&(v.iconSkin="person",v.name=this.getOperationHtml(v),v.click="SNSIMWindow.getInstance().getInvitationWindow().selectToInvite('"+t.attr("id")+"','"+t.attr("name")+"')");g.push(v)}a.loaded=!0;a.loading=!1;this.ztree.addNodes(c.parent,g);this.removeLoadingNode(c.parent)}},this),{parent:a})}};
...
SNSPresenceRender.prototype.updateRosterPresence=function(a,b){var c=b.target,d=c.presence,e=jQuery("li[id$='_"+c.getID()+"'] "+this.rosterPresenceSelector);e.removeClass(this.allStatusClass);e.addClass("snsim_"+d.status);d=jQuery("#narrow_roster_container");if(0<c.groups.length)for(e=0;e<c.groups.length;e++){var g=jQuery("#grouproster_"+c.groups[e].name+"_"+c.id);jQuery("#list_content_"+c.groups[e].name).prepend(g)}c=jQuery("#narrow_roster_"+c.id);d.prepend(c);SNSIMWindow.getInstance().getWideWindow().getTab("roster").updateGroupsInfo()};
...
SNSVCardPanel.prototype.buildInfoListHtml=function(){for(var a="",b=0;b<this.roster.vcard.showpropList.length;b++){var c=this.roster.vcard.showpropList[b],d=SNS_LANG_TEMPLATE["vcard_"+c.replace(".","_")],e=this.roster.vcard[c],g=c.indexOf(".");-1!=g&&(e=this.roster.vcard[c.substr(0,g)][c.substr(g+1)]);e=e?e:"";a+='<li><span style="color: #9fa5a7;">'+d+'</span><span style="color:rgb(80, 80, 80);margin-left: 22px;">'+e+"</span></li>"}return a};
...
SNSIMWindow.prototype._toggleNarrowWideWindow=function(){this._narrowWindow.visible()?(this._wideWindow.show(),this._narrowWindow.hide(),this._chatWindow.transPosition(!0)):(jQuery(this._narrowWindow.rosterContainerSelector).parent().css("height",jQuery(window).height()-80),this._wideWindow.hide(),this._narrowWindow.show(),this._chatWindow.transPosition(!1))};
...
Drag.prototype.Move=function(a){if(this.Lock)this.Stop();else{window.getSelection?window.getSelection().removeAllRanges():document.selection.empty();var b=a.clientX-this._x;a=a.clientY-this._y;if(this.Limit){var c=this.mxLeft,d=this.mxRight,e=this.mxTop,g=this.mxBottom;this._mxContainer&&(c=Math.max(c,0),e=Math.max(e,0),d=Math.min(d,this._mxContainer.clientWidth),g=Math.min(g,this._mxContainer.clientHeight));b=Math.max(Math.min(b,d-this.Drag.offsetWidth),c);a=Math.max(Math.min(a,g-this.Drag.offsetHeight),
...
this._styleHeight;this.Scale&&(this.Ratio=Math.max(this.Ratio,0)||this._styleWidth/this._styleHeight,this._scaleLeft=this._styleLeft+this._styleWidth/2,this._scaleTop=this._styleTop+this._styleHeight/2);if(this.Max){var d=this.mxLeft,e=this.mxRight,g=this.mxTop,s=this.mxBottom;this._mxContainer&&(d=Math.max(d,0),g=Math.max(g,0),e=Math.min(e,this._mxContainer.clientWidth),s=Math.min(s,this._mxContainer.clientHeight));e=Math.max(e,d+(this.Min?this.minWidth:0)+this._borderX);s=Math.max(s,g+(this.Min?
this.minHeight:0)+this._borderY);this._mxSet=function(){this._mxRightWidth=e-this._styleLeft-this._borderX;this._mxDownHeight=s-this._styleTop-this._borderY;this._mxUpHeight=Math.max(this._fixTop-g,this.Min?this.minHeight:0);this._mxLeftWidth=Math.max(this._fixLeft-d,this.Min?this.minWidth:0)};this._mxSet();this.Scale&&(this._mxScaleWidth=2*Math.min(this._scaleLeft-d,e-this._scaleLeft-this._borderX),this._mxScaleHeight=2*Math.min(this._scaleTop-g,s-this._scaleTop-this._borderY))}SNSAvatarCropper.getInstance().addEventHandler(document,
...
!function(a){"function"==typeof define&&define.amd?define(["jquery"],a):"object"==typeof exports?module.exports=a:a(jQuery)}(function(a){function b(b){var g=b||window.event,s=u.call(arguments,1),k=0,p=0,t=0,I=0,L=0,O=0;if(b=a.event.fix(g),b.type="mousewheel","detail"in g&&(t=-1*g.detail),"wheelDelta"in g&&(t=g.wheelDelta),"wheelDeltaY"in g&&(t=g.wheelDeltaY),"wheelDeltaX"in g&&(p=-1*g.wheelDeltaX),"axis"in g&&g.axis===g.HORIZONTAL_AXIS&&(p=-1*t,t=0),k=0===t?p:t,"deltaY"in g&&(t=-1*g.deltaY,k=t),"deltaX"in
g&&(p=g.deltaX,0===t&&(k=-1*p)),0!==t||0!==p){if(1===g.deltaMode)var f=a.data(this,"mousewheel-line-height"),k=k*f,t=t*f,p=p*f;else 2===g.deltaMode&&(f=a.data(this,"mousewheel-page-height"),k*=f,t*=f,p*=f);if(I=Math.max(Math.abs(t),Math.abs(p)),(!e||e>I)&&(e=I,v.settings.adjustOldDeltas&&"mousewheel"===g.type&&0===I%120&&(e/=40)),v.settings.adjustOldDeltas&&"mousewheel"===g.type&&0===I%120&&(k/=40,p/=40,t/=40),k=Math[1<=k?"floor":"ceil"](k/e),p=Math[1<=p?"floor":"ceil"](p/e),t=Math[1<=t?"floor":"ceil"](t/
e),v.settings.normalizeOffset&&this.getBoundingClientRect)g=this.getBoundingClientRect(),L=b.clientX-g.left,O=b.clientY-g.top;return b.deltaX=p,b.deltaY=t,b.deltaFactor=e,b.offsetX=L,b.offsetY=O,b.deltaMode=0,s.unshift(b,k,p,t),d&&clearTimeout(d),d=setTimeout(c,200),(a.event.dispatch||a.event.handle).apply(this,s)}}function c(){e=null}var d,e,g=["wheel","mousewheel","DOMMouseScroll","MozMousePixelScroll"],s="onwheel"in document||9<=document.documentMode?["wheel"]:["mousewheel","DomMouseScroll","MozMousePixelScroll"],
u=Array.prototype.slice;if(a.event.fixHooks)for(var t=g.length;t;)a.event.fixHooks[g[--t]]=a.event.mouseHooks;var v=a.event.special.mousewheel={version:"3.1.12",setup:function(){if(this.addEventListener)for(var c=s.length;c;)this.addEventListener(s[--c],b,!1);else this.onmousewheel=b;a.data(this,"mousewheel-line-height",v.getLineHeight(this));a.data(this,"mousewheel-page-height",v.getPageHeight(this))},teardown:function(){if(this.removeEventListener)for(var c=s.length;c;)this.removeEventListener(s[--c],
...
SNSOrganizationTab.prototype.loadNodeByParent=function(a){if(!a.loaded&&!a.loading){this.addLoadingNode(a);a.loading=!0;var b=new JSJaCIQ;b.setType(SNS_TYPE.GET);b.setTo(a.id+"@org."+SNSApplication.getInstance().getDomain());var c=b.buildNode("query",{xmlns:NS_ORGANIZATION});b.appendNode(c);YYIMChat.send(b,jQuery.proxy(function(b,c){if(b.isError())YYIMChat.log("SNSOrganizationTab.prototype.loadNodeByParent ",0,c);else{for(var g=[],s=jQuery(b.doc.xml).find("item[id]"),u=0;u<s.length;u++){var t=jQuery(s[u]),
v={};v.id=t.attr("id");v.name=t.attr("name");var P=t.attr("isUser");"false"==t.attr("isLeaf")?(v.isParent=!0,v.iconSkin="department"):v.isParent=!1;"true"==P&&(v.iconSkin="person",v.name=this.getOperationHtml(v),v.click="SNSOrganizationPlugin.getInstance().organizationTab.chatWithRoster('"+t.attr("id")+"','"+t.attr("name")+"')");g.push(v)}a.loaded=!0;a.loading=!1;this.ztree.addNodes(c.parent,g);this.removeLoadingNode(c.parent)}},this),{parent:a})}};
...
SNSUploadPlugin.prototype.initImageUpload=function(){jQuery("#auploadImage").bind("click",{_self:this},function(a){jQuery(a.target).find("#"+a.data._self.imageUploadId).trigger("click")});jQuery("#"+this.imageUploadId).bind("change",{_slef:this},function(a){a=SNSIMWindow.getInstance().getChatWindow().getActiveRoster();var b=SNS_CHAT_TYPE.CHAT;a instanceof SNSChatRoom&&(b=SNS_CHAT_TYPE.GROUP_CHAT);b={fileInputId:this.id,to:a.getID(),type:b,success:function(a){var b=new SNSOutMessage(a);b.setImage(a.body.content);
SNSIMWindow.getInstance().getChatWindow().afterSendMessage(b)},error:function(){alert("\u56fe\u7247\u53d1\u9001\u5931\u8d25")}};a.resource&&(b.resource=a.resource);YYIMChat.sendPic(b)})};
SNSUploadPlugin.prototype.initBreakPointUpload=function(){jQuery("#upload").bind("click",{_self:this},function(a){jQuery(a.target).find("#"+a.data._self.breakPointId).trigger("click")});jQuery("#"+this.breakPointId).bind("change",{_slef:this},function(a){a=SNSIMWindow.getInstance().getChatWindow().getActiveRoster();var b=SNS_CHAT_TYPE.CHAT;a instanceof SNSChatRoom&&(b=SNS_CHAT_TYPE.GROUP_CHAT);b={fileInputId:this.id,to:a.getID(),type:b,success:function(a){var b=new SNSOutMessage(a);b.setFile(a.body.content);
SNSIMWindow.getInstance().getChatWindow().afterSendMessage(b)},error:function(){alert("\u6587\u4ef6\u53d1\u9001\u5931\u8d25")},progress:function(a){}};a.resource&&(b.resource=a.resource);YYIMChat.sendFile(b)})};SNSUploadPlugin.getInstance=function(){return SNSUploadPlugin._instance};(new SNSUploadPlugin).start();
...
SNSSpaceRosterProvider.prototype.queryRosterItem=function(a){if(a){var b=SNSSpaceApplication.REST_BASE+"member/search/mid/"+YYIMChat.getUserBareNode()+"/key/"+a.keyword;jQuery.ajax({url:b,dataType:"jsonp",type:"get",success:jQuery.proxy(function(b,d,e){YYIMChat.log("search roster results",3,b);d=[];for(var g in b)e=b[g],d.push({id:String(e.value),name:e.name});SNSCommonUtil.isFunction(a.success)&&a.success(JSON.stringify(d))},this),error:function(a){YYIMChat.log("get roster list error",2,arguments)}})}};
